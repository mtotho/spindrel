...
|---|---|---|---|
| Security Architecture | low/hygiene shipped 2026-05-02 | Low/hygiene tier shipped (CI `pip-audit` + `npm audit`, GitHub Actions pinned by SHA, `deployment_tier_readiness` audit signal). Earlier 2026-05 deep review shipped MCP SSRF guard, encryption fail-fast, tool-result redaction, `/auth/*` rate-limit parity, approval origin_kind awareness, supply-chain signing Phase 1+2. Live queue is container `sudoers` cleanup (deferred — `install_system_package` build-time path migration first). | [[security]] |
