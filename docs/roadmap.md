...
|---|---|---|---|
| Security Architecture | red-team R1+R2 Phase 1 shipped 2026-05-02 | Red-team pass landed two: R1 (integration inbound prompt-injection wrap at `inject_message` + `_enqueue_chat_turn`), R2 (run_script raw-Python egress allowlist via injected `sitecustomize.py` patching `socket.socket.connect` — `audit` default, `enforce` opt-in; Phase 2 OS-level netns deferred). Earlier 2026-05 pass shipped MCP SSRF guard, encryption fail-fast, tool-result redaction, `/auth/*` rate-limit parity, approval origin_kind, supply-chain signing Phase 1+2, low/hygiene tier. Live queue: R3 (`read_container_logs` tier bump), R4 (`propose_config_change` self-mutation), container sudoers (deferred). | [[security]] |
