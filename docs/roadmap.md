...
|---|---|---|---|
| Security Architecture | red-team R1–R4 Phase 1 shipped 2026-05-02 | Red-team pass landed all four: R1 (integration inbound prompt-injection wrap at `inject_message` + `_enqueue_chat_turn`), R2 (run_script raw-Python egress allowlist via injected `sitecustomize.py` — `audit` default, `enforce` opt-in; Phase 2 OS-level netns deferred), R3 (`read_container_logs` + `get_recent_server_errors` bumped to `control_plane`), R4 (autonomous-origin self-mutation of high-impact bot fields refused; every self-mutation audit-logged via `log_self_mutation`; UI badge / cross-bot delegation tracing deferred). Earlier 2026-05 pass shipped MCP SSRF guard, encryption fail-fast, tool-result redaction, `/auth/*` rate-limit parity, approval origin_kind, supply-chain signing Phase 1+2, low/hygiene tier. Live queue: container sudoers (deferred — `install_system_package` build-time path migration first). | [[security]] |
