...
|---|---|---|---|
| Security Architecture | red-team R1+R2+R3 shipped 2026-05-02 | Red-team pass landed three: R1 (integration inbound prompt-injection wrap at `inject_message` + `_enqueue_chat_turn`), R2 (run_script raw-Python egress allowlist via injected `sitecustomize.py` — `audit` default, `enforce` opt-in; Phase 2 OS-level netns deferred), R3 (`read_container_logs` + `get_recent_server_errors` bumped to `control_plane` so they require admin grant; `system_health_*` dedup tools stay `readonly`). Earlier 2026-05 pass shipped MCP SSRF guard, encryption fail-fast, tool-result redaction, `/auth/*` rate-limit parity, approval origin_kind, supply-chain signing Phase 1+2, low/hygiene tier. Live queue: R4 (`propose_config_change` self-mutation), container sudoers (deferred). | [[security]] |
