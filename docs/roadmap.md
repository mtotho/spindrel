...
| Agent Quality Observability | trace/eval standardization started 2026-05-03 | Deterministic post-turn trace auditor emits versioned `agent_quality_audit` findings; Daily Health surfaces counts; `audit_trace_quality` supports admin/scheduled re-audit; `recent_attachment_context` makes carried-forward image context visible; track now anchors OTel/W3C-aligned trace cleanup. | [[agent-quality-observability]] |
