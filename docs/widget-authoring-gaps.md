---
tags: [spindrel, reference, widgets, authoring, dev-panel]
status: reference
updated: 2026-04-18
---
...
Companion to [[widget-dashboard]]. Captures the real unification question that surfaced during P6 planning: **there is no duplicate UI to merge** across `/admin/widget-packages/new`, `/admin/widget-packages/[id]`, and `/widgets/dev` — the gap is a *missing bridge* from the sandbox layer (ephemeral) to the library layer (persistent). This note seeds a future `Track - Widget Authoring UX`.
