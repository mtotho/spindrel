---
title: emit_html_widget reference — modes, bundles, sandbox, auth
summary: Full reference for `emit_html_widget` — inline vs library_ref vs path modes, bundle layout, YAML frontmatter, `widget://` path grammar, CSP sandbox + `extra_csp`, and the bot-scoped auth model.
tags: [widgets, reference, html, sandbox, auth]
---
