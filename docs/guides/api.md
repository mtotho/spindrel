...
The `skills.recommended_now` section is read-only. It points the agent at existing runtime skills with exact first actions such as `get_skill("diagnostics")`, especially for procedural workflows that smaller models may miss. `skills.creation_candidates` names missing procedural coverage that should become a future runtime skill; it does not create, import, enroll, or auto-inject skills.
