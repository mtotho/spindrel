...
| **Heartbeat run** | A narrow-profile agent run (no live chat replay, no ambient injections) that fires without a human turn. Heartbeat definitions live on `channel_heartbeats`; concrete executions are queued through the Task runner so session targeting, traces, and completion handling stay consistent. | cron run, tick |
