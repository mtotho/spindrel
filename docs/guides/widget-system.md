...
| `core/command_center_native` | `channel`, `dashboard` | native widget instance + host-rendered payload | First-party operations board for bot assignments, next heartbeat timing, upcoming work, and recent reports. Convenience UI only; it deliberately does not export prompt context. |
