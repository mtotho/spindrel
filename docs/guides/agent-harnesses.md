...
For the current feature-by-feature audit, see [Claude Code and Codex Harness Parity](harness-parity.md). That matrix tracks native runtime support, Spindrel support, proof screenshots/tests, and the next gaps to close.
...
![Codex native `/cloud` result rendered in chat](../images/harness-codex-native-cloud-result-dark.png)

![Codex native `/approvals` result rendered in chat](../images/harness-codex-native-approvals-result-dark.png)

![Codex native `/apps` result rendered in chat](../images/harness-codex-native-apps-result-dark.png)

![Codex native `/skills` result rendered in chat](../images/harness-codex-native-skills-result-dark.png)

![Codex native `/mcp` result rendered in chat](../images/harness-codex-native-mcp-status-result-dark.png)
...
- `slash_policy.allowed_command_ids` filters `/api/v1/slash-commands?bot_id=...` and `/help`. Passing `session_id` also lets Spindrel merge runtime-reported session inventory, such as Claude SDK `system/init` slash commands, into the picker.
- `native_commands[]` advertises runtime-owned command surfaces. The slash picker shows these as their real names, such as `/plugins`, `/plugin`, `/skills`, `/mcp`, `/diff`, `/resume`, `/status`, and `/doctor`, instead of requiring `/runtime <command>`.
...
`/runtime` remains available as a diagnostic escape hatch, but harness channels should normally type native commands directly. Spindrel resolves the current visible session, routes the native command to the runtime adapter, and renders the result as terminal-like command output with expandable JSON/stdout details. Runtime-native aliases are adapter-owned: for example Codex exposes `/mcp` as an alias for its `mcp-status` app-server method, `/plugin` as an alias for `/plugins`, `/feature` as an alias for `/features`, and `/marketplaces` as an alias for `/marketplace`.
...
Native installs stay runtime-owned. Claude project-local skills live under the harness cwd at `.claude/skills/<name>/SKILL.md`; user/global Claude skills, Claude plugins, Codex plugins, Codex marketplaces, Codex skills config, and Codex MCP config remain in the runtime's own config locations. Spindrel routes, renders, approval-gates, or opens a terminal handoff; it does not mirror these registries into Spindrel skills or plugin tables.

Harness turns persist an `input_manifest` summary in assistant metadata. It records the runtime-visible input item types, selected Spindrel skill ids, attachment summaries, and workspace upload refs without storing inline base64 content. Codex uses this manifest to send native `skill`, `image`, and `localImage` input items where the app-server protocol supports them. Claude Code maps inline and workspace image attachments into Claude SDK image content blocks when they are readable from the harness cwd; slash and project-local skill handling remain SDK-native.
...
| `/compact`, `/new`, `/clear`, and `/context` | Supported | Harness `/compact` triggers native runtime compaction when supported. `/new` and `/clear` open a fresh Spindrel session without deleting the old one or changing the channel primary/default pointer. `/context` is native-first: Claude Code dispatches native `/context` through the SDK when a native session exists, while Codex returns an explicit terminal handoff until its app-server exposes a read-only context method. Spindrel host diagnostics are nested under labeled host context, not presented as native output. |
| Native slash commands | Supported | Type supported runtime-native management commands directly. Codex routes read paths plus supported management paths through app-server JSON-RPC: `/config`, `/config set ...`, `/config requirements`, `/plugins`, `/plugin read ...`, `/plugin uninstall ...`, `/skills`, `/skills enable\|disable ...`, `/mcp`, `/mcp resource read ...`, `/features`, `/features enable\|disable ...`, `/marketplace add\|remove\|upgrade ...`, `/status`, `/hooks`, `/apps`, `/app`, `/fs list\|read\|info ...`, `/resume`, `/resume search ...`, `/cloud`, and `/approvals` config-requirement reads. The Codex schema drift gate verifies the installed binary still exposes the method names Spindrel depends on (`app/list`, `configRequirements/read`, `fs/readFile`, `fs/readDirectory`, `fs/getMetadata`, and `command/exec`); newer optional methods such as `hooks/list`, and entitlement-denied management reads such as a 403 from `app/list`, fall back to terminal handoff. Codex CLI-native flows that still require an interactive/TTY surface or are not exposed by the installed app-server (`/diff`, `/undo`, `/branch`, `/review`, `/prompts`, `/editor`, `/init`, and unsupported argument forms) return terminal handoff with the exact command. Mutating app-server calls are harness approval-gated before execution. Claude Code exposes `/skills`, `/plugin list`, `/mcp list`, `/agents`, `/hooks`, `/status`, and `/doctor` through the installed CLI when available, returning terminal handoff for mutating or TTY-only flows. |
...
| Native input manifest | Supported | Harness turns record a redacted manifest of text-adjacent native inputs. Codex maps `$skill-name` markers to `skill` items when `skills/list` can resolve a path, inline image attachments to `image` items, and channel workspace image uploads to `localImage` items. Claude maps inline and cwd-local workspace image attachments to SDK image content blocks, skipping unreadable/out-of-cwd files with manifest warnings. |
