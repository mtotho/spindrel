...
Harness turns persist an `input_manifest` summary in assistant metadata. It records the runtime-visible input item types, selected Spindrel skill ids, attachment summaries, and workspace upload refs without storing inline base64 content. Codex uses this manifest to send native `skill`, `image`, and `localImage` input items where the app-server protocol supports them; Claude currently records the same manifest for diagnostics while slash and project-local skill handling remain SDK-native.
...
| Native input manifest | Supported | Harness turns record a redacted manifest of text-adjacent native inputs. Codex maps `$skill-name` markers to `skill` items when `skills/list` can resolve a path, inline image attachments to `image` items, and channel workspace image uploads to `localImage` items. |
