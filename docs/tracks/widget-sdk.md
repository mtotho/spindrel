---
tags: [spindrel, track, widgets, sdk, framework]
status: active
...
- `docker run --rm spindrel-test pytest tests/unit/test_widget_py.py tests/unit/test_widget_events.py tests/unit/test_widget_cron.py tests/unit/test_widget_manifest.py tests/unit/test_widget_db.py tests/integration/test_widget_event_dispatch.py tests/integration/test_widget_handler_dispatch.py tests/integration/test_widget_cron.py tests/integration/test_widget_db_dispatch.py` → **110 passed** (up from B.4's 108: +2 `TestNotifyReload`).
...
- `widget_db.py` `_BUILTIN_WIDGET_DIR` uses `parents[2]` but should be `parents[1]` (resolves to a non-existent `spindrel/tools/local/widgets/`). Unrelated latent bug — flagged in Loose Ends, fix out of scope for C.0.
...
- [[widget-dashboard]] — dashboard surface this track builds on
- [[widgets]] — underlying tool-renderer system (YAML templates); this track does NOT touch that
