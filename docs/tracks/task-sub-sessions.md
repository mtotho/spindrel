---
tags: [spindrel, track, architecture]
status: active
updated: 2026-04-22 (session switcher polish follow-up — visible Make main action, lower-chrome attached menu pass; Phase 9 session-aware scratch metadata/history/promotion shipped; prompt/skill alignment shipped for session-aware history; Phase 8 polish — bot sub-session visibility + mobile dock + cross-device scratch; thread dock parity; dashboard scratch handoff clarity; session switcher/menu polish)
---

# Track — Task Sub-Sessions (pipeline-as-chat refactor)

Context replay/compaction/history-policy details referenced here defer to the canonical guide at [Context Management](../../../spindrel/docs/guides/context-management.md).
...
- **Plan execution artifacts** — session plans now carry a lightweight `artifacts` list in the canonical markdown/API shape. The first consumer is widget bundle versioning: when a bot creates/updates/rolls back a `widget://bot/...` or `widget://workspace/...` bundle during an active plan, the accepted plan records a `widget_revision` artifact with ref + revision metadata instead of overloading checklist semantics.
...
- **Session switcher polish follow-up (2026-04-22)** — the promote action was technically present but effectively hidden as an unlabeled icon, especially on the currently viewed session. `ScratchSessionMenu` now exposes a visible low-chrome `Make main` action on session rows, replaces the loud `VIEWING` pill with a softer `Current` chip, moves `Open main chat` into the header action strip, removes the extra standalone body row, and tightens the desktop sheet so it feels attached to the `Sessions` button instead of floating like a detached modal.
...
`~/.claude/plans/snug-wibbling-simon.md`. Session log: `Sessions/spindrel/2026-04-21-02-sub-session-polish.md`.
