---
tags: [spindrel, track, providers, llm, reasoning]
status: active
created: 2026-04-23
...
- New canonical doc `spindrel/docs/guides/providers.md` in the mkdocs nav.
...
- Canonical guide: `spindrel/docs/guides/providers.md` — extended in Phase 5 (capability columns, extra_headers, extra_body / Ollama `num_ctx`) and Phase 6 (daily refresh, Provider Health panel).
...
- Related: [[code-quality]] (god-function split of `_prepare_call_params` is sibling work).
- Related: [[experiments]] (pipeline-layer optimization harness — `/effort` becomes an interesting knob for autoresearch once Phase 2 gates it).
