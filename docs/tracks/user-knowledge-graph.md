...
|---|---|---|
| 0 | Fix KB retrieval prefix scoping in `retrieve_filesystem_context`. Standalone bug fix + `fix-log.md` entry. | not started |
| 0.5 | Knowledge Document primitive: lift Notes infrastructure into a unified service supporting multiple scopes + multi-mode session binding. Refactor Notes to consume it. No end-user behavior change. | not started |
| 1 | Auto-capture pass (review-first, generic types) writing through the unified primitive at user scope. Includes indexing/retrieval, bus event, admin review queue, cross-user leak test. | not started |
...
- Phase 0 first (standalone PR; KB prefix scoping bug fix).
- Phase 0.5 second (substrate refactor; Notes lifts into unified Knowledge Document primitive). Sequential with Phase 1.
