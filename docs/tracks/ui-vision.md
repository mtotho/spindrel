...
Canonical design spec lives in `spindrel/docs/guides/ui-design.md`. This track is the **living companion** — what has been adopted, what is outstanding, and what the next pass looks like. The doc is target; this track is progress.
...
| Area | Status | Notes |
|---|---|---|
| Canonical spec doc | ✅ shipped 2026-04-23 | `spindrel/docs/guides/ui-design.md`, linked from mkdocs Guides nav |
...
| `spindrel-ui` skill | ✅ corrected 2026-04-24 | Project-level at `spindrel/.claude/skills/spindrel-ui/SKILL.md`; gradient/shadow "signature moves" removed so it now follows the canonical guide instead of extending it. |
...
| Admin Bots | ✅ refreshed 2026-04-25 | `/admin/bots` uses dense catalog rows; `/admin/bots/:botId` uses grouped workflow navigation. Memory & Learning plus Workspace & Files now use shared controls, mobile-safe rows, low-chrome hygiene expanders, and `SourceFileInspector` for bot knowledge previews. |
...
- `spindrel/docs/guides/ui-design.md` — the canonical spec (target).
