...
### R4 — `propose_config_change` self-mutation (High, gated by approval)
**Where:** `app/tools/local/propose_config_change.py:280, 349` — `safety_tier="mutating"`, requires approval. But a bot CAN propose changes to its own definition (`delegate_bots`, `tool_retrieval`, allow-lists). Fast-approval by a fatigued operator silently broadens the bot's own authority. **Fix:** detect self-referential proposals and either refuse outright or require a second-bot / human-only approval lane.
