...
created: 2026-04-30
updated: 2026-05-01
summary: Evergreen security track. 2026-05 pass shipped High tier (MCP SSRF guard, encryption fail-fast, tool-result redaction, OAuth/refresh rate limits, approval origin_kind, widget symlink rejection, run_script tightening, backup encryption, supply-chain signing Phase 1) and most of the Medium tier (migration 130 downgrade guard, cross_workspace_access cleanup, file-read untrusted-data wrapping, approval-rule autonomous toggle, widget JWT revocation list); remaining queue is supply-chain signing Phase 2, container sudoers cleanup (deferred — install_system_package depends on it), and Low/hygiene.
---
