...
### R3 — `read_container_logs` defaults to `readonly` (High)
**Where:** `app/tools/local/read_container_logs.py:41-87` calls `@register({...})` with no `safety_tier=` kwarg → defaults to `readonly` per `app/tools/registry.py:65`. Any bot with the tool grant can call it without approval. The result_for_llm goes through the boundary redactor, but `secret_registry` is pattern-scoped — env-dump logs, stack traces with positional secrets, and integration init lines often contain shapes the redactor doesn't catch. **Fix:** bump to `mutating` or `control_plane`; admin-grant only.
