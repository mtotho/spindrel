...
- `cd /home/mtoth/personal/spindrel/ui && npx tsc --noEmit` clean after extracting `ToolRenderersPane` and wiring it into the shared library surface.
...
- `cd /home/mtoth/personal/spindrel/ui && npx tsc --noEmit` reached the targeted file checks and the only surfaced regression in-session (`ToolRenderersPane` stale `tool` reference) was fixed; the wrapper did not return a final clean completion signal afterward.
...
- **Old HTML Notes has now been hard-deleted.** The legacy `app/tools/local/widgets/notes/` HTML bundle is gone rather than compatibility-hidden. New Notes placements go through `notes_native`; old direct refs should be treated as unsupported legacy pins that need replacement.
...
Phase 18 follow-up shipped 2026-04-23 — the native widget lane now has three more first-party widgets and a reusable file-navigation contract instead of Notes/Todo being the only meaningful proof points.
