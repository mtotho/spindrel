...
status: active
created: 2026-04-09
updated: 2026-05-01 (harness parity runner consolidated into one canonical Module)
---
...
4. **Harness invocation seam is wide and untyped.** `_run_harness_turn` is called with ~15 loose scalars (`harness_tool_names`, `harness_skill_ids`, `harness_permission_mode_override`, etc.) that get unpacked across the 13-file `app/services/agent_harnesses/` tree. The app↔integrations seam is clean at the import level (per `feedback_integration_boundary`), but the **shape** of the dispatcher payload is not. Suggested deepening: `RunTurnRequest` (TypedDict / dataclass) in `integrations/sdk.py` becomes the canonical input; `_harness_task_turn_overrides` returns one. Smaller in scope than Items 1-2, but improves cross-seam locality.
