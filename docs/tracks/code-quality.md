...
status: active
created: 2026-04-09
updated: 2026-05-01 (ProjectTaskExecutionContext shipped)
---
...
2. **`project_coding_runs.py` (2,016 LOC) bundles three lifecycles.** Task creation/continuation, review context + finalization, and schedule firing share helpers (`_task_run_config`, `_utcnow`, `_slug`, `_task_summary`) but otherwise have independent control flow and data. Suggested split: `project_coding_run_orchestration` (create + continue), `project_coding_run_review` (review context + finalization), `project_run_schedule` (scheduling + firing); shared utilities to a thin `project_coding_run_lib`. Defer until Item 1 ships — extracting context first clarifies what is left in coding-runs.
