...
status: active
created: 2026-04-09
updated: 2026-05-01 (project_coding_runs split into 4 modules)
---
...
6. **Harness parity runner is fragmented across four entry points.** `scripts/run_harness_parity_live.sh` (~320), `run_harness_parity_local.sh`, `run_harness_parity_local_batch.sh` (~300), and `scripts/agent_e2e_dev.py:cmd_prepare_harness_parity` plus `scripts/harness_parity_junit_skips.py`. Tier ordering is duplicated in `tests/e2e/scenarios/test_harness_live_parity.py:TIER_ORDER`, in `_local_batch.sh`, in inline Python inside `_live.sh`, and implicitly in `agent_e2e_dev.HARNESS_PARITY_RUNTIME_CONFIG`. Suggested deepening: one `tests/e2e/harness/parity_runner.py` Module owns tier registry, preset → slice expansion, environment setup, pytest invocation, parallelization mode, JUnit skip validation. Shell scripts become thin wrappers. Note: `tests/unit/test_agent_e2e_dev.py` (770 LOC, 47 helper-function unit tests) is the "extracted-for-testability without orchestration coverage" anti-pattern; expect it to shrink dramatically.
