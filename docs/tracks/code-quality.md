---
title: Track — Code Quality & Refactoring
summary: Evergreen architecture-depth audit. Tracks shipped deepening passes, the live queue from each fresh scan, and watched candidates that need concrete change pressure before reopening.
tags: [spindrel, track, code-quality]
status: active
created: 2026-04-09
updated: 2026-05-01 (cross-integration structural-twin gate added)
---
# Track — Code Quality & Refactoring

Evergreen audit of spindrel architecture depth, Locality, and drift guards. Old cluster numbers are retained only as historical invariants; live work must come from a fresh verify-first scan of current source.

## Current state (2026-04-29 architecture audit)

Fresh scan used the `improve-codebase-architecture` deep Module lens: Interface, Implementation, Depth, Seam, Adapter, Leverage, and Locality. Architecture/drift slice passed before implementation (`45 passed`); focused harness/startup/slash slice passed after implementation (`37 passed, 21 skipped`). Platform slash cleanup follow-up passed focused slash client/drift tests (`10 passed`), broader slash integration slice (`12 passed, 26 skipped`), and redirected bytecode compile for Slack/Discord slash files. Task-run host follow-up passed the focused task worker unit group (`56 passed, 43 skipped`), task host architecture guard (`4 passed`), task integration profile (`16 skipped`), redirected bytecode compile, and scoped diff check. Task-trigger host follow-up passed architecture guard (`4 passed`), non-cron trigger/task unit slice (`59 passed, 18 skipped`), cron subscription slice under `.venv` (`8 passed`), recovery safety slice (`66 passed, 25 skipped`), integration task profile (`16 skipped`), redirected bytecode compile, and scoped diff check. Task-worker host follow-up passed architecture guard (`4 passed`), recovery safety slice (`73 passed, 43 skipped`), task wrapper/host slice (`28 passed`), task integration profile (`16 skipped`), redirected bytecode compile, and scoped diff check. Task-exec host follow-up passed focused exec-host architecture/behavior tests (`6 passed`), adjacent task wrapper/host slice (`30 passed`), broader task/workflow slice (`79 passed, 43 skipped`), redirected bytecode compile, and scoped diff check. Context-admission follow-up passed focused admission/profile/breakdown/widget/cache tests (`61 passed, 16 skipped`), context assembly integration profile (`25 skipped`), redirected bytecode compile, and scoped diff check.
...
- **Channel workbench controller seam.** `ui/app/(app)/channels/[channelId]/useChannelWorkbenchController.tsx` now owns workspace/file-root/terminal cwd policy, channel panel defaults and layout resolution, palette actions, keyboard/focus shortcuts, overlay close policy, zone pins, spine actions, and dock-entry animation state. The route composes the returned model with header/panel/chat JSX. `ui/src/components/chat/renderArchitecture.test.ts` now blocks these workbench policies from drifting back into the route.
- **Startup bootstrap seam.** `app.services.startup_bootstrap` now owns blocking startup bootstrap phases: migrations, config/provider/startup-secret loading, usage/server config, bot/default-workspace setup, registry seeding, integration dependency install, MCP/tool discovery, orphan cleanup, file/skill/workflow registries, workspace dirs, integration routers, manifest validation, endpoint catalog, and web-UI discovery. `app.main::lifespan` stays focused on FastAPI assembly, log setup, embedding-dimension guard, runtime worker launch, STT warmup, ready-time services, and shutdown. `tests/unit/test_startup_bootstrap_architecture.py` pins the boundary.
- **Platform slash participant scope.** Slack/Discord `/ask` now uses the integration SDK slash client to list the same active target set as web/member routing: primary bot plus configured channel member bots. Slack opens a channel-target modal when invoked without args; Discord autocompletes the target option. Legacy `/todos` platform fetches are retired with a deprecation response, while the backend/API/native Todo widget remain untouched.
- **Task run host seam.** `app.agent.task_run_host` now owns general task execution: session-target resolution, active-lock defer, queued `TURN_STARTED`, task preparation, harness/normal execution, timeout/rate-limit/failure cleanup, persistence, follow-up task creation, and turn completion dispatch. `app.agent.tasks.run_task` remains a compatibility wrapper that passes patchable dependencies from `app.agent.tasks`, preserving existing tests and monkeypatch surfaces. `tests/unit/test_task_run_host_architecture.py` keeps task-run policy out of the wrapper.
- **Task trigger host seam.** `app.agent.task_trigger_host` now owns recurrence parsing, active schedule spawning, due schedule polling, channel subscription firing, due subscription polling, event-trigger filtering, and event-trigger task spawning. `app.agent.tasks` keeps wrapper-sized compatibility functions for existing imports and monkeypatches. `tests/unit/test_task_trigger_host_architecture.py` keeps trigger SQL/task-construction policy out of the worker module.
- **Task worker host seam.** `app.agent.task_worker_host` now owns due-task claiming, stuck-task timeout recovery, stalled-workflow recovery, and the polling worker cadence for task schedules, channel subscriptions, widget/native crons, memory hygiene, and daily health summaries. `app.agent.tasks` keeps wrapper-sized compatibility functions and passes lazy worker-lane dependencies at call time. `tests/unit/test_task_worker_host_architecture.py` keeps worker/recovery SQL and cadence policy out of the task module.
- **Task exec host seam.** `app.agent.task_exec_host` now owns raw exec task orchestration: execution-config resolution, script building, sandbox/workspace/bot-local routing, timeout handling, result formatting, completion recording, turn publishing, and optional parent callback creation. `app.agent.tasks.run_exec_task` remains a wrapper that passes the existing patchable dependency surface at call time. `tests/unit/test_task_exec_host_architecture.py` pins both the wrapper boundary and the key success/failure branches.
- **Context admission seam.** `app.agent.context_admission` now owns optional static context admission: memory-scheme files, channel workspace prompt/file/index admission, plan artifact injection, conversation section/index admission, bot knowledge-base retrieval, workspace RAG, and channel workspace tool shaping. `app.agent.context_assembly` remains the public turn-context coordinator and keeps private compatibility wrappers for existing tests/imports. `tests/unit/test_context_admission_architecture.py` pins the ownership split.

**Live queue after the context-admission pass:**
1. **No active task-module blocker.** Task run, trigger, worker/recovery, and raw exec policy are split behind host Modules. `app.agent.tasks` intentionally remains the compatibility facade for existing imports plus specialized runner dispatch and heartbeat helper callbacks.
2. **Context assembly residuals are lower but still worth watching.** `context_assembly.py` is now a coordinator plus discovery/tool/late-injection stages; the next verify-first pass should compare remaining skill working-set, tool retrieval, and late cache-safe injection pressure against other hot files before choosing another cut.

**Live queue from the 2026-05-01 fresh scan:**

Source: `improve-codebase-architecture` skill applied to the recent Project / Harness / E2E waves. Verified-first: friction below is grounded in current source, not stale diagnoses.

1. **✅ Project task execution context — shipped 2026-05-01.** `app/services/project_task_execution_context.py` is now the canonical Module owning execution context assembly, persistence shape, and read-side accessors for Project tasks. Public Interface: 4 named classmethods (`fresh`, `from_parent`, `review`, `from_task`) plus an `apply_to_task(task, channel)` write helper, typed `@property` accessors, byte-stable `to_persisted ↔ from_task` round-trip. Adapter Protocols (`DevTargetAllocator`, `DependencyStackSpecResolver`, `ContextSource`) wire in slot-in support for Phase 4U/4V/4X follow-on work — per-target dependency stacks, ephemeral preview environments, non-Project task classes — without redesign. Internal seam `port_prober` is monkeypatch-friendly. Typed errors (`MalformedExecutionContextError`, `PortAllocationError`, `MissingPresetError`, `MissingSecretsError`) replace ad-hoc `ValueError`. `project_coding_runs.py` orchestration delegates: `create_project_coding_run`, `continue_project_coding_run`, `create_project_coding_run_review_session`, and `_coding_run_row` route through the new Module. The lazy `from app.services.project_dependency_stacks import project_dependency_stack_spec` inside `_safe_dependency_stack_target` is gone (now top-level). Architecture guard at `tests/unit/test_project_task_execution_context_architecture.py` blocks the lazy import + reverse cycle from coming back, and pins each orchestration entry point to the new seam. Behavior change locked in by user decision: continuation now raises `MalformedExecutionContextError` on bad parent shape instead of silently re-allocating dev_targets — the "always reuse parent verbatim" rule (Phase 4I invariant) is now structural.
2. **✅ `project_coding_runs.py` four-way split — shipped 2026-05-01.** Was 1,950 LOC bundling three lifecycles plus shared read-side utilities. Now four canonical owners: `project_coding_run_lib.py` (822 LOC — shared types, helpers, read endpoints), `project_coding_run_orchestration.py` (274 LOC — create + continue), `project_coding_run_review.py` (678 LOC — review session, finalize, mark-reviewed, cleanup, review-context endpoint), `project_run_schedule.py` (295 LOC — schedule CRUD, firing, listing). `project_coding_runs.py` is now a 112-LOC re-export facade — every public name (`ProjectCodingRunCreate`, `_review_summary`, `allocate_project_run_dev_targets`, `fire_project_coding_run_schedule`, etc.) keeps importing from the same path so routers, the workspace launcher, the trigger host, the project_run_handoff tool, and the existing `tests/unit/test_project_coding_runs.py` import block don't move. Architecture guard at `tests/unit/test_project_coding_run_split_architecture.py` pins the four-module ownership: facade is re-export only (no `def`/`class`/non-`__all__` assigns), each lifecycle is independent of the others, the schedule→orchestration `create_project_coding_run` import is the single allowed cross-lifecycle direction, and lib must not import any lifecycle module. The shipped Phase 1 ProjectTaskExecutionContext deepening removed the duplicated execution-context assembly; this pass cleared the remaining cohesion failure.
3. **`agent_capabilities.py` (2,017 LOC) bundles five concerns.** Tool signature building, endpoint/scope filtering, integration readiness ("doctor"), bot repair workflows, and skill-coverage recommendations. Note: prior session log shows this file grew additively as a designed manifest builder (work-state, runtime-context, agent-status, activity-log, tool-error-contract sections). Per the existing track rule about "no split without concrete change pressure," this is **watched, not current queue** unless a concrete change exposes poor Locality across the five concerns. Reframe candidate: extract `integration_doctor` and `skill_coverage` while keeping the manifest builder as the seam, only when the next manifest section change spans both.
4. **✅ Harness invocation seam typed — shipped 2026-05-01.** `_run_harness_turn` was called with 13 loose keyword scalars (`harness_tool_names`, `harness_skill_ids`, `harness_permission_mode_override`, `harness_attachments`, `harness_model_override`, …) at three callsites (`turn_worker._run_harness_branch_if_needed`, `task_run_host._run_harness_task_if_needed`, `heartbeat`), unpacked again at the next hop (`run_harness_turn` in `agent_harnesses/turn_host.py`). Now one canonical envelope: `app/services/agent_harnesses/turn_request.py` owns frozen `HarnessTurnRequest` dataclass with all 17 host-payload fields and a `with_task_execution_config(ecfg)` method that replaces the dict-returning `_harness_task_turn_overrides` (which was already bit-rotted — its lone test imported from `app.agent.tasks` where the symbol no longer existed). Wrapper `_run_harness_turn` shrunk from 50 LOC of kwarg passthrough to a 13-LOC seam taking one `request` arg. `run_harness_turn` keeps its 9 dependency-injection callable kwargs (deliberate test seams) but unpacks request fields into locals at the top so the 540-LOC body is unchanged. **Note on plan deviation**: the original Item 4 text suggested putting the type in `integrations/sdk.py`. That violates `feedback_integration_boundary` — `app/` cannot import from `integrations/`. The host envelope lives host-side alongside `HarnessTurnContext` in `app/services/agent_harnesses/`. Architecture guard at `tests/unit/test_harness_turn_host_architecture.py` pins five invariants: wrapper LOC ≤ 30 (was ≤ 70), wrapper takes exactly one `request` arg with no kwonly args, `run_harness_turn` takes one positional `request` first and forbids the 13 envelope-field names from appearing as kwonly args, all three production callsites construct `HarnessTurnRequest(...)`, and `turn_host.py` retains ownership of the 8 host-orchestration symbols. New `tests/unit/test_harness_turn_request.py` (8 tests) pins frozen-ness, field set, tuple-coercion of list inputs, and `with_task_execution_config` semantics (dedupe + `report_issue` injection + `bypassPermissions`). `tests/unit/test_harness_task_automation_config.py` migrated to the new shape (was previously broken on `development`). Six `_run_harness_turn` callsites in `tests/unit/test_turn_worker_harness_context.py` rewritten to construct `HarnessTurnRequest(...)`; `tests/unit/test_harness_turn_tags.py` and `tests/integration/test_turn_worker.py` fakes migrated from `**kwargs` to `(request)`. Verification: `pytest tests/unit/test_harness_turn_request.py tests/unit/test_harness_turn_host_architecture.py tests/unit/test_harness_task_automation_config.py tests/unit/test_turn_worker_harness_context.py tests/unit/test_harness_turn_tags.py tests/integration/test_turn_worker.py -q` → 48 passed; broader harness sweep `pytest tests/unit/ -k harness` → 125 passed, 46 skipped (DB-backed Python 3.14 skips).
5. **✅ Cross-integration structural-twin gate — shipped 2026-05-01.** The existing exact-name gate `tests/unit/test_integration_no_duplicate_helpers.py` caught two integrations defining the same private name; it missed the inverse smell — same body, different name. AST-normalized hash audit ran across all 11+ active integrations: anonymize arg/local names, elide constants, hash the body, group by hash, flag any group spanning ≥2 integrations. **In the originally-named trio (`claude_code` / `codex` / `demo_harness`): zero structural twins.** The four prior suspects in the track text — image-block builders, structured tool-result wrappers, diff renderers, tool-use audit formatters — were either already centralized in the prior pass at `app/services/agent_harnesses/tool_results.py` (`build_diff_tool_result`, `unified_diff_from_strings`, `build_text_tool_result`) or correctly diverged (Claude's 70-LOC image builder vs Codex's 5-LOC variant target different protocols). **Across the broader codebase: 27 baseline structural-twin clusters**, dominated by deliberate Discord/Slack/BlueBubbles platform parallels (matching the existing `ALLOWED_DUPLICATES` rationale: same shape by design, lifting requires a shared platform-adapter base — different track) plus the local_companion/ssh and truenas/unifi adapter pairs. Productionized the audit logic into the existing test file as `test_no_unallowlisted_structural_twins` + `test_structural_twin_allowlist_does_not_rot`. New `ALLOWED_STRUCTURAL_TWINS: dict[str, str]` (27 entries) keyed by 16-char AST hash with terse rationale; entries auto-invalidate when a body mutates structurally, forcing a re-review. The companion exact-name gate also picked up `_ask_targets_for_channel` (a discord/slack drift since the last allowlist update); added in the same edit. The one-off `scripts/audit_integration_helpers.py` was deleted — the test now does the same scan every CI run, the script was redundant. **Verification**: `pytest tests/unit/test_integration_no_duplicate_helpers.py -v` → 4 passed. **Track-text deviation**: original Item 5 framing ("audit between claude_code/codex/demo_harness, lift confirmed twins to integrations/sdk.py") didn't apply — the trio was already clean from prior work, so the deliverable became "permanent gate to catch the *next* drift" rather than "lift X helpers today." Logged in commit message and this entry rather than rewriting the original framing, per same-edit cleanup discipline.
6. **✅ Harness parity runner consolidated — shipped 2026-05-01.** Was fragmented across four entry points. Now one canonical Module: `tests/e2e/harness/parity_runner.py` owns the tier registry (`TIER_ORDER`), runtime config (`HARNESS_PARITY_RUNTIME_CONFIG`), env builder (`HarnessEnv` — fail-fast on unset `E2E_PORT`, no shared `:18000` fallback), pytest invocation (`run_tier`), JUnit skip validation (`validate_skips`), tier-route preflight (`validate_tier_requirements`), preparation orchestration (`cmd_prepare`, moved from `agent_e2e_dev.py`), and a CLI dispatch (`live`, `tier-rank`, `tier-at-least`, `validate-routes`, `validate-skips`, `expand-slices`, `is-full-suite`). `tests/e2e/harness/parity_presets.py` owns the slice-preset table + `screenshot_filter_for_selector` (data-only, no orchestration imports). Shell scripts shrunk: `run_harness_parity_live.sh` 412→345 LOC (inline 12-tier dict and `tier_at_least` inline-Python deleted; pytest invocation and JUnit-skips check shell out to the Module), `run_harness_parity_local_batch.sh` 401→300 LOC (78 LOC of inline slice arrays + 44 LOC of `screenshots_for_selector` deleted; bash reads slices via `expand-slices` and keeps native `printf %q` dry-run rendering so existing tests assert byte-for-byte the same output), `harness_parity_junit_skips.py` 55→47 LOC (now a thin re-export shim — `from tests.e2e.harness.parity_runner import validate_skips as unexpected_skips` keeps `tests/unit/test_harness_parity_scripts.py:8` working). `scripts/agent_e2e_dev.py:HARNESS_PARITY_RUNTIME_CONFIG` is now a top-level re-export and `cmd_prepare_harness_parity` is a 5-line shim. `tests/e2e/scenarios/test_harness_live_parity.py` imports `TIER_ORDER` from the Module instead of redefining it. Architecture guard at `tests/unit/test_harness_parity_runner_architecture.py` pins five invariants: TIER_ORDER has one Python definition (no shells redefine it as a dict literal or associative array), DEFAULT_ALLOWED_SKIP_REGEX has one definition, parity_presets does not import parity_runner (no cycle), HARNESS_PARITY_RUNTIME_CONFIG has one definition (`agent_e2e_dev` keeps a re-export, not a redefine). Test attrition: `tests/unit/test_agent_e2e_dev.py` 37→35 (lost two `cmd_prepare_harness_parity` orchestration tests to the new file); `tests/unit/test_harness_parity_runner.py` adds 21 tests (2 migrated + 19 new pinning tier registry, validate_tier_requirements, validate_skips, HarnessEnv strict mode, presets, run_tier command shape); architecture guard adds 5. The plan's broader test deletion premise ("~10 utility tests delete because helpers move to private surface") did not apply — per the plan's Phase 1 design the agent_e2e_dev helpers stay where they are (still called by `cmd_doctor`, `cmd_write_env`, etc.); deleting tests of still-callable helpers would lose coverage. Verification: `pytest tests/unit/test_harness_parity_runner.py tests/unit/test_harness_parity_runner_architecture.py tests/unit/test_harness_parity_scripts.py tests/unit/test_agent_e2e_dev.py -q` → 71 passed; `bash -n` on all three shell scripts passed; `--list` and `--dry-run` matrices for `smoke|slash|bridge|sdk|ui|fast|deep|all` presets emit identical output to pre-refactor. Plan file at `~/.claude/plans/shiny-enchanting-clover.md`.

**Considered, dropped from the queue:**
- **Merge four task hosts into one TaskOrchestrator** — contradicts the 2026-04-29 shipped decision (`task_run_host` / `task_exec_host` / `task_trigger_host` / `task_worker_host` were extracted as four intentional seams with architecture-test guards). Reopen only if concrete drift evidence shows the four-way split is now harming Locality.
- **Extract `ClaudeCliDiagnostics` from `integrations/claude_code/harness.py`** — the explore agent inferred this from the `app/tools/local/e2e_tests.py` deletion; verification shows that tool was simply removed, not folded inline. Keep as a watched candidate if the harness's CLI-management subsection grows further.
- **Workspace spatial aggregator** (one hook over the 9 spatial hooks) — real friction, but lower-priority than Items 1-6 and the 2026-04-30 spatial-canvas pass already softened the surface.
...
- **Channel route residual size** — top-tab, session-pane, and workbench controller seams are shipped. Reopen only for a concrete change that shows remaining route policy is too local to JSX composition.
...
## Maintenance pass (2026-05-01)

- **Harness parity runner consolidation shipped.** Was fragmented across four entry points (`run_harness_parity_live.sh` 412 LOC, `run_harness_parity_local.sh` 139 LOC, `run_harness_parity_local_batch.sh` 401 LOC, `agent_e2e_dev.py:cmd_prepare_harness_parity` ~110 LOC, `harness_parity_junit_skips.py` 55 LOC) with `TIER_ORDER` duplicated in three places (the Python parity test, a bash dict in live.sh, and the slice-list embeddings in local_batch.sh). Same shape as the 2026-04-29 task-host split and the just-shipped `project_coding_runs` split: one canonical Module per concern, original entry points become thin Adapters, an architecture guard pins ownership. New canonical owner: `tests/e2e/harness/parity_runner.py` owns the tier registry, runtime config (`HARNESS_PARITY_RUNTIME_CONFIG`), env builder (`HarnessEnv` — fail-fast on unset `E2E_PORT`, respects the new agent-owned-port strategy with no shared `:18000` fallback), pytest invocation (`run_tier`), JUnit skip validation (`validate_skips`), tier-route preflight (`validate_tier_requirements`), preparation orchestration (`cmd_prepare`, moved from `agent_e2e_dev.py`), and a CLI dispatch (`live`, `tier-rank`, `tier-at-least`, `validate-routes`, `validate-skips`, `expand-slices`, `is-full-suite`). Sibling `tests/e2e/harness/parity_presets.py` owns the slice-preset table + `screenshot_filter_for_selector` mapping — data-only, no orchestration imports. Shell scripts shrunk: `run_harness_parity_live.sh` 412→345 LOC (inline 12-tier preflight dict and inline `tier_at_least` Python deleted; pytest invocation and JUnit skip gate shell out to the Module), `run_harness_parity_local_batch.sh` 401→300 LOC (78 LOC of inline slice arrays + 44 LOC of `screenshots_for_selector` deleted; bash reads slices via `expand-slices` while keeping its native `printf %q` dry-run rendering so the existing byte-for-byte assertions in `test_harness_parity_scripts.py` pass without test edits), `harness_parity_junit_skips.py` 55→47 LOC (re-export shim — `from tests.e2e.harness.parity_runner import validate_skips as unexpected_skips` keeps `test_harness_parity_scripts.py:8` working). `scripts/agent_e2e_dev.py:HARNESS_PARITY_RUNTIME_CONFIG` is now a top-level re-export from the Module; `cmd_prepare_harness_parity` is a 5-line shim that lazily imports `parity_runner.cmd_prepare(args)` (no cycle — `agent_e2e_dev` imports the Module at top level, the Module imports back lazily inside `cmd_prepare`). `tests/e2e/scenarios/test_harness_live_parity.py:47` flipped from a 12-line inline dict to `from tests.e2e.harness.parity_runner import TIER_ORDER`. Architecture guard at `tests/unit/test_harness_parity_runner_architecture.py` pins five invariants: TIER_ORDER has one Python definition; shell scripts must not redefine it as a dict literal or associative array; DEFAULT_ALLOWED_SKIP_REGEX has one definition; parity_presets must not import parity_runner (no cycle); HARNESS_PARITY_RUNTIME_CONFIG has one definition (`agent_e2e_dev` keeps a re-export, not a redefine). Test attrition: 2 migrated cmd_prepare orchestration tests left `test_agent_e2e_dev.py` (37→35); `test_harness_parity_runner.py` added 21 tests (2 migrated + 19 new pinning tier registry, validate_tier_requirements, validate_skips, HarnessEnv strict mode, preset slices, run_tier command shape); architecture guard added 5. The plan's broader test-deletion premise ("~10 utility tests delete because helpers move to private surface") did not apply — per Phase 1's design the helpers in `agent_e2e_dev.py` stayed put (still called by `cmd_doctor`, `cmd_write_env`, `cmd_start_native_api`, etc.); deleting tests of still-callable helpers would lose coverage for no architectural reason. Verification: `pytest tests/unit/test_harness_parity_runner.py tests/unit/test_harness_parity_runner_architecture.py tests/unit/test_harness_parity_scripts.py tests/unit/test_agent_e2e_dev.py -q` → 71 passed; `bash -n` on all three shell scripts passed; `--list` and `--dry-run` matrices for every preset emit byte-equivalent output to pre-refactor (verified visually for `smoke|slash|bridge|sdk|ui|all`). Plan file at `~/.claude/plans/shiny-enchanting-clover.md`. Out-of-scope follow-ups (deferred): the bash live.sh SSH-aware `wait_for_server_health` + admin/channels-curl preflight stays in bash per the plan ("keeping it bash keeps fast-fail close to the user's environment"); migrating it to Python would buy ~120 LOC at the cost of the bash health-check fast-fail.
- **`project_coding_runs.py` split shipped.** Was 1,950 LOC bundling three lifecycles plus shared read-side utilities. Same shape as the 2026-04-29 task-host split: one canonical owner per concern, original module becomes a re-export facade preserving every public name, architecture guard pins ownership. New canonical owners: `app/services/project_coding_run_lib.py` (822 LOC) for shared types / helpers / read endpoints, `app/services/project_coding_run_orchestration.py` (274 LOC) for create + continue, `app/services/project_coding_run_review.py` (678 LOC) for review session creation / finalize / mark-reviewed / cleanup / review-context, `app/services/project_run_schedule.py` (295 LOC) for schedule CRUD / firing / listing. `app/services/project_coding_runs.py` is now a 112-LOC re-export facade — every public name keeps importing from the same path, so callers (`app/routers/api_v1_projects.py`'s 21-name import block, `app/services/workspace_attention.py`, `app/agent/task_trigger_host.py`, `app/tools/local/project_run_handoff.py`, `tests/unit/test_project_coding_runs.py`) don't move. The single allowed cross-lifecycle import is `schedule.fire_project_coding_run_schedule → orchestration.create_project_coding_run` (a schedule fire spawns a fresh coding run); the architecture guard pins this direction. `_attach_task_machine_grant` was promoted to lib so all three lifecycles use it without lazy imports across owners. Architecture guard at `tests/unit/test_project_coding_run_split_architecture.py` enforces facade-only shape (no `def`/`class`/non-`__all__` assigns), inter-lifecycle independence, schedule's single allowed cross-lifecycle direction, and lib's no-lifecycle-imports constraint. The two ProjectTaskExecutionContext architecture guards that scanned `project_coding_runs.py` (`test_create_project_coding_run_uses_execution_context_fresh`, `test_continue_project_coding_run_uses_from_parent`, `test_create_review_session_uses_execution_context_review`) were updated to scan the new owner modules. Verification (local Python 3.14 / aiosqlite ≤0.22 — DB-backed tests auto-skip per the conftest guard): `pytest tests/unit/test_project_coding_run_split_architecture.py tests/unit/test_project_coding_runs.py tests/unit/test_project_task_execution_context.py tests/unit/test_project_task_execution_context_architecture.py tests/unit/test_project_runtime.py tests/unit/test_project_dependency_stacks.py tests/unit/test_run_presets.py` → 34 passed, 39 skipped. Plan file at `~/.claude/plans/shiny-enchanting-clover.md`. Out-of-scope follow-up: delete now-redundant compatibility-only helpers (`_execution_config_from_preset`, `_dev_target_env`, `allocate_project_run_dev_targets`, `_safe_runtime_target`, `_safe_dependency_stack_target`, `_dev_target_specs`, `_is_port_listening`) once test imports migrate.
- **ProjectTaskExecutionContext deepening shipped.** Four shallow Modules (`project_coding_runs.py` (2,016 LOC), `project_runtime.py`, `project_dependency_stacks.py`, `run_presets.py`) collectively assembled one logical concept — execution context for a Project task — through duplicated calls. `app/services/project_task_execution_context.py` is now the single seam. Verified pre-deepening friction in current source: `load_project_runtime_environment` was called from four separate sites in `project_coding_runs.py` (lines 583, 1001, 1551, 1629); the lazy `from app.services.project_dependency_stacks import project_dependency_stack_spec` inside `_safe_dependency_stack_target` (line 194) signaled circular coupling; `dev_targets` flowed through 21+ sites with `_dev_target_env(dev_targets)` and the raw list stored side-by-side in execution_config. Design (per `improve-codebase-architecture` skill, parallel sub-agent interface design pass): four named classmethods (`fresh` / `from_parent` / `review` / `from_task`), Protocol-based adapters (`DevTargetAllocator`, `DependencyStackSpecResolver`, `ContextSource`) wired for the active Phase 4U/4V/4X follow-on work, typed errors (`MalformedExecutionContextError` etc.), and the `to_persisted ↔ from_task` round-trip property as the deletion test. Continuation behavior change: raises `MalformedExecutionContextError` on bad parent shape instead of silently re-allocating — the "always reuse parent verbatim" rule (Phase 4I invariant) is now structural rather than convention. Architecture guard at `tests/unit/test_project_task_execution_context_architecture.py` blocks the lazy import + reverse cycle and pins each orchestration entry point to the new seam. Verification (local Python 3.14 / aiosqlite ≤0.22 — DB-backed tests auto-skip per the conftest guard, exercise on Python 3.12 runtime): `pytest tests/unit/test_project_task_execution_context.py tests/unit/test_project_task_execution_context_architecture.py tests/unit/test_project_coding_runs.py -v` → 19 passed, 36 skipped (DB-backed). Plan file at `~/.claude/plans/shiny-enchanting-clover.md`.

## Maintenance pass (2026-04-29)

- **Context admission deepening shipped.** `app/agent/context_admission.py` is now the static context-admission Module for memory-scheme file injection, channel workspace helper/context/index admission, plan artifact injection, conversation sections/section index, bot knowledge-base retrieval, workspace RAG, and channel workspace tool shaping. `app/agent/context_assembly.py` keeps the public `assemble_context` coordinator and private compatibility wrappers for the moved helpers, preserving existing test/import surfaces. The pass also made WorkSurface resolution failure fall back to legacy channel workspace admission instead of aborting the whole channel workspace stage. Added `tests/unit/test_context_admission_architecture.py` to pin the ownership split. Verification: redirected-bytecode compile passed; focused admission suite passed (`20 passed`); profile/breakdown architecture slice passed (`22 passed`); public generator/widget/cache wrapper slice passed (`23 passed, 16 skipped`); combined targeted context slice passed (`61 passed, 16 skipped`); context assembly integration profile skipped locally (`25 skipped`); scoped `git diff --check` passed.
- **Task exec host deepening shipped.** `app/agent/task_exec_host.py` is now the host Module for raw exec tasks: execution config parsing, UUID normalization, script construction, sandbox-instance/workspace/bot-local routing, timeout failure publishing, generic failure recording, success result formatting, completion recording, TURN_ENDED publishing, and optional parent callback task creation. `app/agent/tasks.py::run_exec_task` is wrapper-sized and passes the existing patchable dependencies into the host at call time. Added `tests/unit/test_task_exec_host_architecture.py` to guard the wrapper/host split and cover workspace success, timeout failure, and generic no-sandbox failure behavior. Verification: redirected-bytecode compile passed; focused exec-host suite passed (`6 passed`); adjacent task wrapper/host slice passed (`30 passed`, with the existing aiosqlite event-loop warning); broader task/workflow slice passed (`79 passed, 43 skipped`); scoped `git diff --check` passed.
- **Task worker host deepening shipped.** `app/agent/task_worker_host.py` is now the host Module for due-task claiming, stuck-task timeout recovery, stalled-workflow recovery, and the polling worker cadence across schedules, channel subscriptions, widget/native crons, memory hygiene, and daily health summaries. `app/agent/tasks.py` keeps wrapper-sized compatibility functions for `fetch_due_tasks`, `recover_stuck_tasks`, `recover_stalled_workflow_runs`, and `task_worker`, passing existing patchable dependencies at call time. Added `tests/unit/test_task_worker_host_architecture.py` to pin the wrapper/host split. Verification: redirected-bytecode compile passed; architecture guard passed (`4 passed`); recovery safety slice passed (`73 passed, 43 skipped`); task wrapper/host slice passed (`28 passed`); task integration profile skipped under local settings (`16 skipped`); scoped `git diff --check` passed.
- **Task trigger host deepening shipped.** `app/agent/task_trigger_host.py` is now the host Module for recurrence parsing, active schedule spawning, due schedule polling, channel subscription firing, due subscription polling, event-trigger filtering, and event-trigger task spawning. `app/agent/tasks.py` keeps wrapper-sized compatibility functions that pass `async_session` and private helper callbacks at call time, preserving existing tests that monkeypatch `app.agent.tasks.async_session`, `_spawn_from_schedule`, or `_fire_subscription`. Added `tests/unit/test_task_trigger_host_architecture.py` to pin the wrapper/host split. Verification: redirected-bytecode compile passed; architecture guard passed (`4 passed`); non-cron trigger/task unit slice passed (`59 passed, 18 skipped`, with existing local async warnings); cron subscription slice passed under `.venv` (`8 passed`) because host Python lacks `croniter`; recovery safety slice passed (`66 passed, 25 skipped`); task integration profile skipped under local settings (`16 skipped`); scoped `git diff --check` passed.
- **Task run host deepening shipped.** `app/agent/task_run_host.py` is now the host Module for general task execution, while `app/agent/tasks.py::run_task` stays wrapper-sized and passes the existing `tasks.py` patchable dependencies into the host at call time. The pass moved session target resolution, session-lock defer, queued task-start publish, task preparation, harness-backed execution, normal agent execution, timeout/rate-limit handling, failure cleanup, result persistence, follow-up creation, and turn-ended dispatch out of the task worker. `_is_pipeline_child` and `_fire_task_complete` now ignore malformed/non-dict callback configs so test doubles and bad rows do not accidentally enable every optional callback path. Added `tests/unit/test_task_run_host_architecture.py` to pin the wrapper/host boundary. Verification: redirected-bytecode compile passed; task host architecture guard passed (`4 passed`); focused task worker unit group passed (`56 passed, 43 skipped`, with the existing AsyncMock expunge warning); task integration profile skipped under local settings (`16 skipped`); scoped `git diff --check` passed.
- **Startup bootstrap deepening shipped.** `app/services/startup_bootstrap.py` is now the blocking startup bootstrap Module, while `app/main.py::lifespan` is back to application/runtime lifecycle concerns: logging, embedding-dimension validation, startup bootstrap invocation, worker/background-service launch, STT warmup, ready runtime services, and shutdown. The extraction keeps the endpoint-catalog and integration-router registration close to bootstrap ordering but avoids importing FastAPI in the service. Added `tests/unit/test_startup_bootstrap_architecture.py` to guard the ownership split. Verification: redirected-bytecode compile passed for startup files; startup/env/runtime architecture slice passed (`17 passed`); FastAPI/background boundary slice passed (`11 passed, 8 skipped`); `python -c "from app.main import app; print('ok')"` completed, with a pre-existing widget-authoring tool import warning captured separately in Loose Ends; scoped `git diff --check` passed. The broader endpoint-catalog slice still has a stale `POST /api/v1/workspaces/{workspace_id}/pull` expectation captured in Loose Ends.
- **Channel workbench controller deepening shipped.** `ui/app/(app)/channels/[channelId]/useChannelWorkbenchController.tsx` is now the channel workbench/chrome policy Module: workspace/project/file roots, terminal request/cwd construction, panel preference defaults, layout resolution, mobile drawer state, zone pins, collapsed-spine actions, palette registrations, keyboard/focus shortcuts, overlay close policy, and dock-entry animation state. `index.tsx` keeps layout composition and passes the controller model into existing header, drawer, OmniPanel, file viewer, dock, and chat-pane components. Added a render architecture guard so `useFileBrowserStore`, palette registration, panel layout resolution, terminal cwd building, workbench callbacks, and focus-layout event handling cannot silently drift back into the route. Verification: `cd ui && npx tsc -p tsconfig.chat-tests.json --pretty false` passed; `node --test .chat-test-dist/ui/src/components/chat/renderArchitecture.test.js` passed; `npx tsc --noEmit --pretty false` passed; scoped `git diff --check` passed.
...
- **Chat enqueue boundary deepening shipped.** `app/routers/chat/_routes.py::_enqueue_chat_turn` is now a stage-only coordinator over web-user metadata, audio/input preparation, sub-session routing, normal channel/session/member context resolution, passive/throttle/pause short-circuits, attachment preallocation/id threading, start-or-queue turn handling, and success-only attention response marking. Public `POST /chat` and `/chat/stream` route behavior/response shapes remain unchanged, and existing test patch surfaces stay available in `app.routers.chat._routes`. Added an AST guard so task/message persistence, audio decoding, attachment creation, start-turn, busy-queue, throttle, and settings policy cannot silently drift back into the coordinator. Verification: redirected-bytecode compile passed; route import passed; `pytest tests/unit/test_chat_routes_architecture.py -q` passed; focused chat/audio/attachment integration selections skipped under the local profile; the session-status busy selector matched no tests.
- **Turn stream event bridge deepening shipped.** `app/services/turn_event_emit.py::emit_run_stream_events` is now a publish-only coordinator over pure typed-event builders for token/thinking/tool, approval, context/memory, LLM status, and skill auto-inject run-stream events. The public `emit_run_stream_events(...)` Interface and one-for-one passthrough behavior are unchanged; `publish_typed` remains centralized in the coordinator. Added missing bridge coverage for memory bootstrap, LLM retry/fallback/cooldown/error, and auto-inject mappings plus architecture guards that block payload construction/publish calls from drifting back into the wrapper. Verification: redirected-bytecode compile passed; `pytest tests/unit/test_turn_event_emit.py -q` passed with 27 tests; channel-event domain/contract slice passed with 14 tests; focused turn-worker metadata integration slice passed with 1 test.
- **Run stream orchestration deepening shipped.** `app/agent/loop.py::run_stream` is now a stage-only coordinator over stream setup, context assembly event forwarding, post-assembly budget/rerank/skill/usage handling, and tool-loop emission. The extracted owner `app/agent/loop_stream.py` preserves the public `run_stream(...)` Interface, event ordering, usage-limit early return, synthetic auto-injected skill tool pairs, outermost delegation buffering, nested stream passthrough, and existing patch surfaces by dependency injection. Added architecture and behavior guards so budget/rerank/usage/delegation policy cannot silently drift back into the route-level coordinator. Verification: redirected-bytecode compile passed; focused run-stream/delegation architecture slice passed with 6 tests.
- **Workspace map-state read-model deepening shipped.** `app/services/workspace_map_state.py::build_workspace_map_state` is now a stage-only coordinator over seed loading, channel room attachments, widget subscription/source projection, heartbeat projection, upcoming/recent activity, attention warnings, trace-error mapping with Daily Health fallback, bot summaries, landmark summaries, and response finalization. The `/api/v1/workspace/spatial/map-state` public shape and `"existing_primitives"` source remain unchanged. Added architecture and target-resolution guards so the coordinator cannot silently absorb SQL/projection policy again. Verification: redirected-bytecode compile passed; `pytest tests/unit/test_workspace_map_state_cues.py -q` passed with 5 tests; local spatial integration slice skipped under the current profile.
...
- **Slash-command integration seam shipped.** Slack/Discord server-owned slash commands moved off duplicated platform implementations and onto a reusable integration SDK client for the backend command host. `/context`, `/compact`, and `/model` now flow through `app.services.slash_commands`, so harness-bound platform channels use harness diagnostics, native compaction, and session-scoped harness model settings while non-harness channels use channel model overrides. Platform-specific commands and list/contents views remain local. Added SDK client coverage, static drift gates, and backend model status/clear regressions.
- **Context diagnostics read-model deepening shipped.** `app/services/context_breakdown.py` split budget trace lookup, stale-after-compaction fallback, scope/session validation, runtime preview category extraction, live-history stats, pruning savings, compaction state, rerank state, effective settings, and response serialization into private seams. `compute_context_breakdown` dropped from 432 LOC to 80 LOC; `fetch_latest_context_budget` dropped from 208 LOC to 106 LOC. Admin/public context-breakdown routes now share `context_breakdown_response`; public output intentionally omits admin-only `effective_settings`. Added coordinator-size and router serializer drift guards. Verification: redirected-bytecode compile passed; context read-model unit slice passed with 4 tests; adjacent budget/assembly/event unit slice passed with 63 tests; local DB-backed context integration slices skipped under the current profile.
...
- **Integration package-root facade deepening shipped.** `integrations/__init__.py` is now a compatibility facade instead of the owning module for discovery/catalog policy. Side-effectful runtime loading moved to `integrations/discovery.py`; manifest-to-SETUP/process helpers moved to `integrations/manifest_setup.py`; side-effect-free catalog projections now live behind `app/services/integration_catalog.py` for setup status, sidebar/dashboard/web UI/process/docker/activation/binding/event discovery. Production private-helper imports moved to the owning modules, `list_bindable_integrations()` no longer calls side-effectful `discover_integrations()`, and the unreachable legacy setup-status block after the compatibility return was deleted. Added architecture guards for facade size, private-helper import drift, and bindable-list side effects. Verification: redirected-bytecode compile passed; focused integration catalog/admin/channel/process/deps/reload/manifests slice passed with 139 tests / 34 skipped; integration activation/binding/subprocess/docker smoke passed with 8 tests / 40 skipped; docs drift and `from app.main import app` checks passed.
...
- `mirror-spindrel-docs-bulk.sh` after vault edits.
- Per `feedback_targeted_test_runs`: scoped pytest, never `pytest tests/ integrations/` in Docker.
- Per `feedback_git_workflow`: spindrel uses manual commit; commit only when user directs.
