...
status: active
created: 2026-04-09
updated: 2026-05-01 (harness invocation seam typed via HarnessTurnRequest envelope)
---
...
5. **Duplicate-helper audit between `integrations/claude_code/` and `integrations/codex/`, `integrations/demo_harness/`.** The existing gate `tests/unit/test_integration_no_duplicate_helpers.py` catches exact duplicates; structural near-twins (image-block builders, structured tool-result wrappers, diff renderers, tool-use audit formatters) likely exist across harnesses without being caught. Bounded one-off audit; lift confirmed twins to `integrations/sdk.py`.
