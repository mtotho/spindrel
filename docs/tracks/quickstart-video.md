---
tags: [spindrel, track, docs, screenshots, video, active]
status: active
updated: 2026-04-25 (B3 playwright recording + Remotion renderer both landed)
---
...
| B2.5 | B | Remotion renderer (parallel ship-target to MoviePy) | **done** — shipped 2026-04-25; `scripts/screenshots/video/remotion/` (Node project, React+TS, `@remotion/cli@4.0.451`) consumes the same `quickstart.yml` storyboard via `compose_remotion.py`, which resolves all `still` / `doc_hero` / `doc_callout` scenes to PNGs on disk, copies them into the Remotion `public/` dir, serializes scenes+meta to `scripts/screenshots/.cache/remotion-props.json`, and shells out to `npx remotion render`. CLI gate: `python -m scripts.screenshots video build --renderer {moviepy,remotion}` (default `moviepy` keeps the fast preview path; `remotion` writes to `<slug>-remotion.mp4`). Synthetic title-card scenes are spliced in Python (no Section component on the JS side) so the React layer just iterates a flat scene list. Scenes use `staticFile()` not bare URLs — Remotion's bundle-time public/ enumeration won't find files written after bundle start. KenBurnsImg is CSS-`transform`-driven (translate+scale on the pre-fitted image) instead of per-frame PIL crop; gives smoother sub-pixel motion than MoviePy's PIL.LANCZOS pass. Smoke test: stills-only quickstart → 28.6s 1920×1080 h264 mp4 in ~50s render time at 6× concurrency on this laptop, vs ~3min for the MoviePy path. |
| B3 | B | Playwright interaction recordings + manual slots | **done — kind=playwright** shipped 2026-04-25; `scripts/screenshots/video/recording/playwright_record.py` drives `sync_playwright` with `record_video_dir`, runs a typed action vocab (`goto/wait_ms/wait_for/click/hover/fill/type/press/scroll_to/scroll_y/js`), then transcodes the webm to h264 mp4 via ffmpeg with `tpad=stop_mode=clone` so the clip is exactly `scene.duration` even when actions finish early. Auth is auto-detected: any relative-path `goto` triggers a `SpindrelClient.login(...)` to mint tokens which get seeded into the browser's `localStorage` under `agent-auth` (mirrors `capture/browser._storage_init_script`). Both renderers wired: `clips/playwright_action.py` loads the recording as a `VideoFileClip` (MoviePy path), and `compose_remotion.py` copies the mp4 into `remotion/public/` and emits a new `kind: "video"` payload that the React layer renders via `<OffthreadVideo>` (new `scenes/Video.tsx`). Smoke verified end-to-end: 5s example.com recording → 1280×720 h264 mp4. `manual` slot still later. |
| B4 | B | TTS voice-over (ElevenLabs skill is the natural fit) | later |
