...
status: active
created: 2026-04-26
updated: 2026-05-02 (Phase 8 — Parity Loop Automation shipped: supervised-iteration recipe + spindrel-harness-parity-loop child + /parity-loop slash + live config audit helper)
---
# Track - Harness SDK
...
## Phase 8 - Parity Loop Automation — shipped 2026-05-02

The dream "create a Project coding run that drives parity to green and walks away" path now has substrate. This phase ships **repo-dev-only** plumbing — nothing in this phase ships to other Spindrel installs. The loop reuses already-shipping product substrate (Project coding runs, schedules, `check_project_coding_run_loop_continuation`, receipts, bounded budgets); it does not add new product surface.

**What landed (all repo-dev surface):**

- **`scripts/spindrel_live_config_audit.py`** — read-only audit of a live Spindrel server's Project Factory orchestration config (factory-state health, concurrency cap, intake convention, `.spindrel/WORKFLOW.md` presence, runtime registry name resolution). `--apply` PATCHes the project for default intake config and drops a starter WORKFLOW.md when missing. Discovers the API key via `docker exec agent-server-agent-server-1 printenv API_KEY` when run on spindrel-bot, or accepts `--api-key`. Covered by `tests/unit/test_spindrel_live_config_audit.py` (8 tests, mocked HTTP).
- **`scripts/harness_parity_loop_iteration.py`** — single-iteration runner. Refuses to run inside `SPINDREL_PROJECT_RUN_GUARD=1` (Project task agents must not invoke `prepare-harness-parity` recursively per the e2e skill's lines 296-301). Reads `scratch/agent-e2e/harness-parity.env`, invokes `./scripts/run_harness_parity_local_batch.sh --tier <tier>` with `--junitxml`, parses the JUnit, cross-references the parity matrix in `docs/guides/harness-parity.md` for spec-row context and the script's `_OWNING_HEURISTICS` for module routing, writes the gap report under `.spindrel/audits/harness-parity/<YYYYMMDD-HHMM>.md`, prints the report path on stdout. Defers ALL stack lifecycle to `.agents/skills/spindrel-e2e-development/SKILL.md`. Covered by `tests/unit/test_harness_parity_loop_iteration.py` (18 tests, no docker / no live stack).
- **Generic supervised-loop recipe** at `.agents/skills/spindrel-supervised-loop/SKILL.md` — Spindrel-internal recipe for "run a suite, parse failures, fix one, commit, repeat within budget" loops. Names the abstract steps and the contract a child skill must declare (`mode: plan_heavy | review_heavy`, `suite_runner`, `spec_source`, `owning_module_heuristic`, `gap_artifact_path`). Encodes the **vibe-engineering mode matrix** (frontend × new-features = review-heavy with screenshot evidence; everything else plan-heavy on test green). Lives in `.agents/skills/` because every concrete loop here targets Spindrel's own internals; not a runtime feature for other installs.
- **Concrete parity loop instance** at `.agents/skills/spindrel-harness-parity-loop/SKILL.md` — Spindrel harness-specific child of the recipe. Slot-ins: `mode=plan_heavy`, `suite_runner=python scripts/harness_parity_loop_iteration.py --tier <tier>`, `spec_source=docs/guides/harness-parity.md`, owning-module heuristic per the script, artifact path `.spindrel/audits/harness-parity/<ts>.md`.
- **Loop continuation reused, not reinvented**: each iteration publishes `ProjectRunReceipt.metadata.loop_decision = {decision: "continue" | "stop", reason: "...", fixed: <test_id>, remaining: <count>, mode: <mode>, evidence: {...}}` per the existing `app/services/project_coding_run_loops.py::check_project_coding_run_loop_continuation` contract. Bounded budget enforced by 4BB.4's continuation policy.
- **Kickoff via existing product API, not a new slash command**: the user creates a Project coding run (or a `project_coding_run_schedule` for recurring) on their own Spindrel instance with the agent prompt naming the repo-local skill and a `loop_policy: {max_iterations, max_time_minutes}`. A small repo-dev installer script (queued as 8.1) seeds that row on the user's instance via the live API so it is one command, not click-around-the-UI. **No `/parity-loop` slash command, no new pydantic preset, no UI surface.** Other Spindrel installs never see any of this.

**Live config gaps observed during plan mode** (recorded for follow-up, not yet fixed): `GET /api/v1/projects/{id}/factory-state` returns 500 on the live server; `GET /api/v1/runtimes/claude_code/capabilities` returns 404 (Codex resolves; Claude does not — runtime registry name drift); the canonical Spindrel project has `concurrency.max_concurrent_runs=null`, `intake.kind="unset"`, `repo_workflow.present=false`. The audit helper surfaces these as suggestions when run; fixing them is a separate slice the user can take with `--apply`.

**Adjacent retro skill**: `.agents/skills/spindrel-dev-retro/SKILL.md` is a **separate, independent** repo-dev skill that audits Spindrel itself — process health, product health, skill effectiveness, memory health. Output: `docs/audits/dev-retro-<YYYYMMDD>.md`. Not parity-specific. Not loop-fired. The user invokes it deliberately, end-of-week.

**What is NOT in this phase**: any product-surface slash command, any new pydantic preset, a "Parity dashboard" page, auto-merging parity-fix PRs, cron scheduling, or speculative edits to `integrations/codex/harness.py` / `integrations/claude_code/harness.py`. The loop FINDS the gaps; iteration N fixes whichever the loop reports.

**Phase 8.1 (queued)**: `scripts/spindrel_install_repo_loop.py` — repo-dev installer that POSTs a Project coding run (one-shot via `--once`) or a `project_coding_run_schedule` row (recurring via `--cron <expr>`) to the user's Spindrel instance, with the agent prompt referencing `.agents/skills/spindrel-harness-parity-loop/SKILL.md` and the loop_policy budget. Same API-key discovery pattern as `spindrel_live_config_audit.py`. Repo-dev surface only; never ships in the product.
