---
tags: [spindrel, track, widgets, dashboard, dev-panel]
status: active
...
>
> **2026-04-30 channel freeform canvas repair.** The channel dashboard no longer exposes an in-canvas zoom/fit/spatial control cluster. A single header lock button toggles a real camera lock and restores the native dashboard home view at `scale=1`; while locked, pan/zoom drift is suppressed and container resize recomputes the home camera. The home camera frames the full rail-center-dock lane group instead of centering only the middle lane. The freeform dashboard surface is full-bleed inside the page body, uses grab/grabbing background panning when unlocked, and no longer relies on DnD-kit for freeform tile movement; widgets move from the explicit move handle with pointer capture. Live drag preview is continuous in free grid space and only snaps on drop, header drops fit the full top-center lane again, and final move/resize previews stay mounted until the layout write resolves to avoid bounce-back flicker. Resize affordances are hover/active-only again. New invariant: the channel dashboard is a viewing surface first; debug/navigation controls must not sit on top of the canvas, and "home" must mean native dashboard scale with no clipped lanes rather than fit-to-page shrink.
>
> Same-day correction after user validation: freeform move targeting now follows the pointer instead of the tile's bounding box. This makes side-lane escape less sticky, lets the top-center lane accept and auto-fit dragged widgets, and restores the default grid footprint when a rail/dock widget is dropped into grid space instead of preserving the side-lane's one-column width. New invariant: zone changes should be decided by where the user's hand is, while the tile's final dimensions should adapt to the destination zone.
>
> Follow-up correction after more manual drag testing: manual grid moves no longer run collision-search relocation on drop, because the nearest "open" cell could be above/left of the release point and made southeast drags bounce backward. Manual moves now snap to the released grid cell; auto-placement remains reserved for newly added pins. Header-zone drops also use the full header height instead of collapsing to a one-row sliver.
>
...
>
> **2026-04-24 presentation default follow-up.** The header-zone shell's unset/default presentation now resolves to Glass instead of the old solid surface. Settings present Glass as the default choice, while the legacy `default` value remains accepted as a compatibility alias for the solid Surface option. New invariant: a channel with no explicit `header_backdrop_mode` renders top-center widgets with translucent glass chrome.
>
> Same-day dashboard follow-up after that: the channel dashboard's unified body canvas now keeps a viewport-height floor even when the lowest actual pin sits much higher on the page. New invariant: the visible grid/drop surface should extend to the page bottom by default; sparse dashboards must not collapse the editable canvas to the last placed widget.
>
...
>
> **2026-04-23 native widget responsive-system follow-up.** The context tracker treatment is now the native-widget baseline rather than a one-off. Native widget specs/contracts expose `layout_hints`, and the remaining dashboard/chat native widgets split into two responsive families: data cards (`usage_forecast`, `upcoming_activity`, `todo`) genuinely recompose across `compact / standard / tall / wide`, while interaction-heavy widgets (`notes`, `pinned_files`, `channel_files`, `plan_questions`) collapse to summary cards with a single primary action in tight slots and only expand into full editors/browsers/forms when the tile is large enough. Live native widget bodies no longer duplicate host-owned titles. New invariant: dashboard/chat chrome owns the title lane; `layout_hints` now seed initial placement and clamp host-side size/resize bounds; native widgets still recompose from measured host geometry rather than from hint shape alone.

> **2026-04-25 Agent Smell native dashboard widget.** `core/agent_smell_native` adds a dashboard-scoped investigation card that ranks bots by live trace/tool-call smell signals, uses configured bot avatars/icons in the row list, and opens evidence via the global trace inspector. New invariant: operational native widgets should keep durable instance state minimal when host telemetry is the source of truth.
>
> **2026-04-24 widget-pin stability follow-up.** `layout_hints` are placement/default-size hints, not permanent resize locks. Chip-family widgets still seed as compact `4x1` header pins when they follow their preferred zone, but explicit/cross-zone moves use the destination zone's default size and resize bounds. Tool actions from dashboard pins now post the enclosing `dashboard_pin_id`; after the mutation, the backend refreshes and persists the pin's own `state_poll` envelope instead of returning the action tool's transient envelope. Shared dashboard identity also includes stable binding config such as `widget_config.entity_id` so two Home Assistant pins with the same generic live-state label do not cross-update or appear to change identities.
>
...
> **2026-04-23 pinned-files widget replacement follow-up.** Legacy `channel.config.pinned_panels[]` is no longer the source of truth for channel file pins. Pin/unpin now targets a single hidden native widget instance, `core/pinned_files_native`, scoped to the channel and auto-surfaced in the channel dashboard `dock` zone on first pin. The old bespoke `PinnedPanelsRail` renderer is gone; chat and dashboard both render pinned files through the normal widget/dock pipeline, and the Files tab derives `Pin to channel` state from that widget's stored `pinned_files[]` state instead of channel config. New invariant: a channel owns at most one pinned-files widget instance; deleting its dashboard pin clears the stored pinned-file list, while unpinning the last file leaves the widget present but empty.
...
Related: [[widgets]] (widget DX/robustness — the underlying system this surface consumes), [[widget-authoring]] (reference), [[widget-authoring-gaps]] (future `Track - Widget Authoring UX` seed).
...
| P13 | Multi-canvas channel dashboard (replaces P12 mental model) | **The P12 positional model was invisible** — users couldn't predict which chat surface a widget would land on without opening a per-pin drawer. Replaced with four real canvases on the channel dashboard, each matching a chat-side surface 1:1: **Floating Header Rail · Rail · Main Grid · Dock**. `widget_dashboard_pins.zone` (enum `rail/header/dock/grid`, NOT NULL) is authored by which canvas the tile lives in; migration 226 backfills existing channel pins by inlining the old classifier rules once + rewriting coords to canvas-local. Runtime classifier + `rail_zone_cols` / `dock_right_cols` preset fields deleted. Cross-canvas moves via per-tile `ZoneChip` dropdown (extracted to its own file to avoid circular imports). Bucket names shortened on the API: `rail / header / dock` (dropped `dock_right` / `header_chip`). Follow-up on 2026-04-23: `header` is no longer a fake singleton slot; it is a floating two-row overlay rail with normal tile coords, and `preferred_zone: chip` now resolves there with `4x1` defaults. Fixed a React #310 hook-order bug in `EditPinDrawer` (useMemo below an early return) that was crashing the pencil button. 45/45 targeted tests green at initial ship; later floating-header follow-up still needs broader manual QA. Plan: `~/.claude/plans/crispy-wishing-sparkle.md`. | **done** (2026-04-20, follow-up 2026-04-23) |
...
- Session log: `vault/Sessions/spindrel/2026-04-18-10-widget-dashboard-dev-panel-p1.md`.
...
- Session log: `vault/Sessions/spindrel/2026-04-18-11-widget-dashboard-p2.md`.
...
- **v2 — full `output_schema` on tools**: deferred P0-2 from [[widgets]]. Broader scope — enables static `{{var}}` validation against shipped templates and machine-readable tool output contracts. This is the "real" version; `result_mime_type` is a strict subset of it.
