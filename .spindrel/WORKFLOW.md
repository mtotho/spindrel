...
| Private session continuity | `~/personal/vault/Sessions/spindrel/` | Personal/operator memory; do not duplicate into repo docs. |
...
Receipts and session logs are evidence. They should link back to the plan,
track, or audit that owns the durable state instead of expanding the track.
