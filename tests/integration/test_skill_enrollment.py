...
# ---------------------------------------------------------------------------
# Conditional auto-enrollment (source="auto")
# ---------------------------------------------------------------------------

class TestConditionalEnrollment:
    """The new 'auto' source covers condition-driven enrollments like the
    workspace-bot starter pack (workspace_member, channel_workspaces, docker_stacks
    when a bot joins a shared workspace). Verifies the Literal accepts the new
    value end-to-end and that enroll_many propagates source='auto' correctly.
    """

    async def test_enroll_many_with_auto_source(self, patched_session, db_session):
        from app.services.skill_enrollment import (
            enroll_many,
            get_enrollments,
            invalidate_enrolled_cache,
        ...
        await _create_bot(db_session, "ws_bot1")
        for sid in ("workspace_member", "channel_workspaces", "docker_stacks"):
            await _create_skill(db_session, sid)
        invalidate_enrolled_cache()
        ...
        rows = await get_enrollments("ws_bot1")
        assert len(rows) == 3
        # All three rows must carry the new 'auto' source — proves it round-trips
        # through both the dialect-branched insert and the read path.
        assert {r.source for r in rows} == {"auto"}
        assert {r.skill_id for r in rows} == {
            "workspace_member", "channel_workspaces", "docker_stacks",
        }

    async def test_auto_enrollment_idempotent(self, patched_session, db_session):
        """Re-running the conditional enrollment (e.g. bot re-added to workspace)
        must not duplicate rows or change the original source from 'auto'."""
        from app.services.skill_enrollment import (
            enroll_many,
            get_enrollments,
            invalidate_enrolled_cache,
        ...
        await _create_bot(db_session, "ws_bot2")
        await _create_skill(db_session, "workspace_member")
        invalidate_enrolled_cache()

        n1 = await enroll_many("ws_bot2", ["workspace_member"], source="auto")
        assert n1 == 1

        # Re-enroll — should be a no-op
        n2 = await enroll_many("ws_bot2", ["workspace_member"], source="auto")
        assert n2 == 0

        rows = await get_enrollments("ws_bot2")
        assert len(rows) == 1
        assert rows[0].source == "auto"

    async def test_post_endpoint_accepts_auto_source(self, client, patched_session, db_session):
        """Bug 4 from session 11 locked the source Literal down. Adding 'auto'
        means the POST endpoint must now accept it without 422-ing."""
        await _create_bot(db_session, "ws_bot3")
        await _create_skill(db_session, "workspace_member")

        resp = await client.post(
            "/api/v1/admin/bots/ws_bot3/enrolled-skills",
            headers={**AUTH_HEADERS, "Content-Type": "application/json"},
            json={"skill_id": "workspace_member", "source": "auto"},
        )
        assert resp.status_code == 201, resp.text

    async def test_post_endpoint_still_rejects_garbage_source(self, client, patched_session, db_session):
        """Regression check: extending the Literal must not accidentally widen it
        to accept arbitrary strings."""
        await _create_bot(db_session, "ws_bot4")
        await _create_skill(db_session, "workspace_member")

        resp = await client.post(
            "/api/v1/admin/bots/ws_bot4/enrolled-skills",
            headers={**AUTH_HEADERS, "Content-Type": "application/json"},
            json={"skill_id": "workspace_member", "source": "definitely_not_a_source"},
        )
        assert resp.status_code == 422
