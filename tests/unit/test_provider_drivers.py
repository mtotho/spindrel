...
class TestDriverCapabilities:
    ...
    @pytest.mark.asyncio
    async def test_openai_subscription_uses_seeded_catalog_without_oauth(self):
        """list_models returns the local seed list so the admin dropdown is
        available before the user completes Connect.
        """
    ...
    @pytest.mark.asyncio
    async def test_openai_subscription_model_listing_does_not_probe_codex_models(self):
        """The Codex OAuth base is proven for /responses, not /models."""
