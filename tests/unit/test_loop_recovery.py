...
@pytest.mark.asyncio
async def test_plan_mode_structured_question_card_widens_pruned_tool_allowlist():
    async def _fail_no_tool(**kwargs):
        raise AssertionError("plan-mode question-card recovery should continue to tool iteration")
        yield {}
    ...
    effective_allowed = {"get_skill"}

    outputs, _ = await _collect(
        accumulated_msg=msg,
        state=state,
        tools_param=[{"type": "function", "function": {"name": "get_skill"}}],
    ...
    assert "ask_plan_questions" in effective_allowed
    assert state.messages[-1]["tool_calls"] == msg.tool_calls
