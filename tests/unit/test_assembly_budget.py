...
class TestAssemblyBudgetTight:
    ...
    async def test_focused_chat_keeps_operator_pins_but_filters_auto_baseline_pins(self):
        ...
        patches = _assembly_patches() + [
            patch("app.agent.tool_surface.retrieval._all_tool_schemas_by_name", new_callable=AsyncMock, return_value=mock_schemas),
            patch("app.agent.tool_surface.retrieval.retrieve_tools", new_callable=AsyncMock, return_value=(
                [mock_schemas["web_search"]], 0.8, [("web_search", 0.8)],
            )),
            metadata_schema_patch,
            patch("app.agent.tool_surface.retrieval.auto_injected_pin_names", return_value=frozenset({
                "file", "get_tool_info",
