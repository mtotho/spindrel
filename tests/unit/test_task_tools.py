...
class TestCreateTask:
    ...
    async def test_create_with_template_link(self):
        ...
        tpl = MagicMock()
        tpl.id = uuid.uuid4()
        tpl.name = "my_template"

        db = AsyncMock()
        db.add = MagicMock()
        db.commit = AsyncMock()
        db.refresh = AsyncMock()
        db.new = set()  # not newly created

        # Mock _resolve_template to return existing
        cm = self._mock_async_session(db)

        # Mock for duplicate detection (no dup)
        dup_result = MagicMock()
        dup_result.scalar_one_or_none.return_value = None
        db.execute = AsyncMock(return_value=dup_result)

        with patch("app.tools.local.tasks.async_session", return_value=cm), \
             patch("app.tools.local.tasks._resolve_template", new_callable=AsyncMock, return_value=tpl), \
             patch("app.tools.local.tasks.current_bot_id") as mock_bot, \
             patch("app.tools.local.tasks.current_session_id") as mock_sid, \
             patch("app.tools.local.tasks.current_channel_id") as mock_cid, \
             patch("app.tools.local.tasks.current_client_id") as mock_client, \
            ...
            result = await create_task(prompt="do stuff", prompt_template="my_template")
            assert "Linked to template 'my_template'" in result
    ...
    async def test_create_duplicate_detection(self):
        ...
        dup_result = MagicMock()
        dup_result.scalar_one_or_none.return_value = existing_task_id
        db.execute = AsyncMock(return_value=dup_result)
        ...
        with patch("app.tools.local.tasks.async_session", return_value=cm), \
            ...
            result = await create_task(prompt="do stuff", prompt_template="dup_tpl")
            assert "Warning:" in result
            assert str(existing_task_id) in result
...
class TestUpdateTask:
    ...
    @pytest.mark.asyncio
    async def test_unlink_template(self):
        from app.tools.local.tasks import update_task
        ...
        task.id = task_id
        task.status = "pending"
        task.prompt_template_id = uuid.uuid4()
        ...
        with patch("app.tools.local.tasks.async_session", return_value=cm):
            result = await update_task(task_id=str(task_id), prompt_template=None)
            assert "template unlinked" in result
            assert task.prompt_template_id is None

    @pytest.mark.asyncio
    async def test_link_template(self):
        from app.tools.local.tasks import update_task
        ...
        with patch("app.tools.local.tasks.async_session", return_value=cm), \
             patch("app.tools.local.tasks._resolve_template", new_callable=AsyncMock, return_value=tpl):
            result = await update_task(task_id=str(task_id), prompt_template="my_tpl")
            assert "linked template 'my_tpl'" in result
