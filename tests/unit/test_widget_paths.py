...
class TestIsWidgetUri:
    def test_recognizes_core(self):
        assert is_widget_uri("widget://core/notes/index.html")
...
class TestResolveCore:
    def test_resolves_index_html(self):
        path, scope, name, ro = resolve_widget_uri(
            "widget://core/notes/index.html", ws_root=None, shared_root=None,
        )
        assert scope == "core"
        assert name == "notes"
        assert ro is True
        assert path.endswith("/widgets/notes/index.html")

    def test_resolves_bare_bundle_dir(self):
        path, _scope, _name, _ro = resolve_widget_uri(
            "widget://core/notes", ws_root=None, shared_root=None,
        )
        assert path.endswith("/widgets/notes")

    def test_traversal_to_sibling_bundle_blocked(self):
        with pytest.raises(ValueError, match="escapes bundle"):
            resolve_widget_uri(
                "widget://core/notes/../other", ws_root=None, shared_root=None,
            )

    def test_traversal_via_slashdot_blocked(self):
        with pytest.raises(ValueError, match="escapes bundle"):
            resolve_widget_uri(
                "widget://core/notes/../../etc/passwd",
