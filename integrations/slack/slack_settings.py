...
def _fetch_slack_config_sync() -> dict:
    """Synchronously fetch Slack config — only used as cold-start fallback."""
...
async def _fetch_slack_config_async() -> dict:
...
def get_slack_config() -> dict:
    """Return cached Slack config, falling back to sync fetch if cache is cold."""
    global _config_cache, _config_cache_ts
    now = time.monotonic()
    if now - _config_cache_ts > _CONFIG_TTL:
        _apply_fresh_config(_fetch_slack_config_sync())
