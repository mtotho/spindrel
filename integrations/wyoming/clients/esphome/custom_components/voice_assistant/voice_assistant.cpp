...
#ifdef USE_SPEAKER
      if (this->speaker_ != nullptr) {
        // [Spindrel patch] Reallocate buffers if freed by a brief IDLE transition.
        // In push-to-talk mode, button release → request_stop() → IDLE happens before
        // our STT_VAD_END event arrives. IDLE calls deallocate_buffers_() which frees
        // speaker_buffer_. When STREAMING_RESPONSE then tries to read into it, crash.
        if (this->speaker_buffer_ == nullptr) {
          ESP_LOGW(TAG, "Speaker buffer was deallocated, reallocating for STREAMING_RESPONSE");
          if (!this->allocate_buffers_()) {
