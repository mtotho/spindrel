...
const AGENT_SERVER_URL = (
  process.env.AGENT_SERVER_URL || "http://host.docker.internal:8000"
).replace(/\/$/, "");
...
app.get("/api/files/channels", (_req, res) => {
  const channels: Array<{ id: string; display_name?: string; workspace_type: string }> = [];

  // Scan shared workspaces: shared/*/channels/*/
  const scanWorkspace = (wsPath: string, wsType: string) => {
    ...
    for (const entry of fs.readdirSync(channelsDir, { withFileTypes: true })) {
      ...
      const info: { id: string; display_name?: string; workspace_type: string } = {
        id: entry.name,
      ...
      if (fs.existsSync(infoPath)) {
        ...
        if (match) info.display_name = match[1].trim();
      }
      channels.push(info);
    }
  };
...
app.get("/api/files/channels/:channelId/files", (req, res) => {
  ...
  const files: Array<{ name: string; path: string; size: number; modified_at: number; section: string }> = [];
...
app.get("/api/files/channels/:channelId/content", (req, res) => {
  ...
  const channelDir = findChannelDir(channelId);
  if (!channelDir) return res.status(404).json({ error: "Channel not found" });
...
app.get("/api/files/channels/:channelId/logs", (req, res) => {
  ...
  const channelDir = findChannelDir(channelId);
  if (!channelDir) return res.json({ logs: [] });
...
function findChannelDir(channelId: string): string | null {
  // Security check
  if (channelId.includes("..") || channelId.includes("/")) return null;
