...
export default function ChannelDetail() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <Link to="/" className="text-xs text-content-dim hover:text-content-muted transition-colors">
          &larr; Overview
        </Link>
        <div className="flex items-center gap-3 mt-1">
          <h1 className="text-2xl font-bold text-content" title={channelId}>
            ...
            <button
              ...
              className={`px-2.5 py-1 text-[10px] rounded-full transition-colors ${
                channel.is_member
                  ...
                  : "bg-surface-3 text-content-muted hover:bg-accent/15 hover:text-accent-hover"
              }`}
            >
        ...
        <div className="flex items-center gap-3 mt-0.5">
          {channel?.bot_name && (
            <p className="text-sm text-content-dim">{channel.bot_name}</p>
          )}
          {channelId && (
            <Link
              to="/kanban"
      ...
      <div className="flex-1 min-w-0 bg-surface-1 rounded-xl border border-surface-3 p-4 overflow-y-auto max-h-[calc(100vh-16rem)]">
        {selectedFile ? (
          ...
          <div className="flex items-center justify-center h-full min-h-[200px]">
            <p className="text-sm text-content-dim">Select a file to view.</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="space-y-4">
      {logs.map((log) => (
        <div key={`${channelId}-${log.date}`} className="bg-surface-1 rounded-xl border border-surface-3 p-4">
          <h3 className="text-sm font-medium text-content mb-2">{log.date}</h3>
  ...
  return (
    <div className="space-y-4">
      ...
      <ContextSection title="Template" badge={data.schema.template_name || undefined}>
        {data.schema.content ? (
          <pre className="text-xs text-content-muted font-mono whitespace-pre-wrap overflow-x-auto">
            {data.schema.content}
          </pre>
        ) : (
          <p className="text-xs text-content-dim italic">No template assigned</p>
        )}
      </ContextSection>

      <ContextSection title={`Workspace Files (${data.files.length})`}>
        {data.files.length === 0 ? (
          <p className="text-xs text-content-dim italic">No workspace files</p>
        ) : (
          <div className="space-y-1">
            {data.files.map((f) => (
              <div key={f.path} className="flex items-center gap-2 text-xs py-1">
                <span className="text-content-muted flex-1 truncate">{f.name}</span>
                <span className="text-content-dim">{f.section}</span>
                <span className="text-content-dim">{(f.size / 1024).toFixed(1)}KB</span>
              </div>
            ))}
          </div>
        )}
      ...
      <ContextSection title={`Tool Calls (${data.tool_calls.length})`}>
        {data.tool_calls.length === 0 ? (
          <p className="text-xs text-content-dim italic">No recent tool calls</p>
        ) : (
      ...
      <ContextSection title={`Trace Events (${data.trace_events.length})`}>
        {data.trace_events.length === 0 ? (
          <p className="text-xs text-content-dim italic">No recent trace events</p>
        ) : (
          <div className="space-y-2">
  ...
  return (
    <div className="bg-surface-2 rounded-xl border border-surface-3 overflow-hidden">
      ...
      >
        ...
        <span className="text-xs text-content-dim">{open ? "\u25B2" : "\u25BC"}</span>
      </button>
...
function ConfigRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-3 py-1">
      <span className="text-xs text-content-dim w-28 flex-shrink-0">{label}</span>
      <span className="text-xs text-content-muted break-all">{value}</span>
    </div>
  );
}
...
function ToolCallRow({ tc }: { tc: ChannelContext["tool_calls"][0] }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span className={`text-xs font-medium ${tc.error ? "text-red-400" : "text-content"}`}>
          {tc.tool_name}
        </span>
        <span className="text-[10px] text-content-dim">{tc.tool_type}</span>
        {tc.duration_ms !== null && <span className="text-[10px] text-content-dim">{tc.duration_ms}ms</span>}
      </div>
      {tc.created_at && <p className="text-[10px] text-content-dim mt-0.5">{new Date(tc.created_at).toLocaleString()}</p>}
      {expanded && (
        <div className="mt-2 pt-2 border-t border-surface-3 space-y-2">
          <div>
            <span className="text-[10px] text-content-dim block mb-0.5">Arguments</span>
            <pre className="text-[10px] text-content-muted font-mono whitespace-pre-wrap bg-surface-0 rounded p-2 overflow-x-auto">
              {JSON.stringify(tc.arguments, null, 2)}
            </pre>
          </div>
          <div>
            <span className="text-[10px] text-content-dim block mb-0.5">Result</span>
            <pre className="text-[10px] text-content-muted font-mono whitespace-pre-wrap bg-surface-0 rounded p-2 overflow-x-auto max-h-48 overflow-y-auto">
              {tc.result}
            </pre>
          </div>
          {tc.error && (
...
function TraceRow({ te }: { te: ChannelContext["trace_events"][0] }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span className="text-xs font-medium text-content">{te.event_type}</span>
        {te.event_name && <span className="text-[10px] text-content-dim">{te.event_name}</span>}
        {te.duration_ms !== null && <span className="text-[10px] text-content-dim">{te.duration_ms}ms</span>}
      </div>
      {te.created_at && <p className="text-[10px] text-content-dim mt-0.5">{new Date(te.created_at).toLocaleString()}</p>}
      {expanded && te.data && (
