...
status: active
updated: 2026-04-25 (P3a static widget tiles + pin-to-canvas action shipped — backend embeds pin payload in node responses, three static widget zoom levels, "Pin to canvas" button next to "Add to dashboard". P3b live iframe + gesture shield deferred to next session.)
---
...
| P3b — Widget tiles (live iframe + shield) | ⬜ | Live iframe at zoom ≥ 0.6 + viewport/zoom culling (only render iframe inside visible tiles, *but* don't remount on entering/leaving viewport — keepalive via `InteractiveHtmlRenderer.tsx`'s patterns). Iframe gesture handling: drag-handle chrome strip is always canvas-pan-eligible; iframe body is pointer-events-blocked by a transparent shield until the tile is "activated" (click); Esc / click-outside deactivates. The shield is the heavy lift; everything below the shield is reused from the existing widget host plumbing (envelope, iframe auth via `window.spindrel.api`, contract snapshots). |
