...
External agent harnesses are becoming a first-class runtime lane rather than a one-off Claude Code bridge. **Phase 3 (harness approvals) and Phase 4 (runtime controls/settings/slash UX) shipped 2026-04-26**. Phase 5 is in progress: session-scoped host hints, harness `/compact` + `/context`, heartbeat hints, harness settings cleanup, Spindrel tool bridge, durable `AskUserQuestion` cards, and persisted harness tool breadcrumbs. Codex and skill bridge work remain. See [[Track - Harness SDK]].
