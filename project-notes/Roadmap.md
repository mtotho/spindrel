...
External agent harnesses are becoming a first-class runtime lane rather than a one-off Claude Code bridge. **Phase 3 (harness approvals) shipped 2026-04-26** — per-session approval modes (`bypassPermissions`/`acceptEdits`/`default`/`plan`), live + orphan approval cards with tool-specific arg previews, `Approve all this turn` per-turn bypass, `expired` decision wired through, header mode-cycle pill + harness runtime badge, integration boundary fixed via `integrations.sdk` re-exports. Phase 4 (runtime capability/settings for model/effort/slash-command filtering), Phase 5 (Codex on the same contract), and later opt-in Spindrel tool/skill bridges remain. See [[Track - Harness SDK]].
