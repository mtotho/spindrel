...
>
> **2026-04-30 channel freeform canvas repair.** The channel dashboard no longer exposes an in-canvas zoom/fit/spatial control cluster. A single header lock button restores the native dashboard home view at `scale=1`, and the canvas stays locked to that native framing until the user pans or zooms. The freeform dashboard surface is full-bleed inside the page body, uses grab/grabbing background panning, and no longer relies on DnD-kit for freeform tile movement; widgets move from the explicit move handle with pointer capture and commit one snapped layout patch. Resize affordances are hover/active-only again. New invariant: the channel dashboard is a viewing surface first; debug/navigation controls must not sit on top of the canvas, and "home" must mean native dashboard scale rather than fit-to-page shrink.
>
