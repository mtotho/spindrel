...
status: active
updated: 2026-04-23
last-shipped: Phase N (N.1 widget preset 7 + N.2 envelope 5 + N.2 FIX + N.3 channel-skill 9 + N.4 session_plan 13 + N.4 FIX + N.5 rerank 9 + N.6 approval lifecycle 12 + N.6 FIX x2 + broken-neighbor revived) 2026-04-23
...
totals: ~1000+ tests across Phases 0–N; 9 real bugs fixed in code, 4 pinned + logged to [[Loose Ends]]
---
...
**Phase N yield**: 55 tests (7 + 5 + 9 + 13 + 9 + 12), 4 real bugs fixed same-commit (N.2 native envelope crash + N.4 dedupe dict-stringify + N.6 ToolCall import + N.6 datetime import), 1 new drift-pin carried to [[Loose Ends]] (N.3 channel-skill cache staleness), 2 pre-existing broken tests revived (N.2 dashboard-pin channel-seed + N.6 orphan-pointer creation).

## Candidate next cycles (ranked backlog, not scheduled)

Pick one per Phase N+ cycle.

| # | Surface | Why it's high-yield |
|---|---|---|
| N.7 | Context-assembly cache TTL drift broadening | New test file is drift-pin only; multi-profile cache invalidation not covered |
| N.8 | Outbox drainer → `channel_events.publish` concurrent drain ordering | `outbox_drainer_worker` flagged uncovered in Coverage Gaps; concurrent-actor seam |
