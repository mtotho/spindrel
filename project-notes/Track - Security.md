...
3. **Security audit deepening.** Continue read-only checks for local-machine leases, widget DB action surfaces, bot-authored skill/widget writable roots, and deployment-tier exposure; inbound callback auth/replay contracts now have an initial audit signal.
