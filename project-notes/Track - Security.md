...
3. **Security audit deepening.** Continue read-only checks for widget DB action surfaces, bot-authored skill/widget writable roots, and deployment-tier exposure; inbound callback auth/replay contracts and local-machine/browser-control now have initial audit signals.
