...
- 2026-04-28 Codex command-output rendering fix: live safe-write sessions showed Codex native `commandExecution` rows persisted without a `ToolResultEnvelope`, so stdout disappeared after refresh even though the CLI showed it. The Codex event adapter now buffers `item/commandOutput/delta` and emits an inline `text/plain` envelope on command completion. Diff envelopes from `fileChange` remain unchanged.
