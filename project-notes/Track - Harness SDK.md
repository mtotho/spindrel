...
status: active
created: 2026-04-26
updated: 2026-04-29 (Codex native slash parity expansion)
---
# Track - Harness SDK
...
- 2026-04-30 terminal-tier redeploy verification: full deployed `terminal` tier on `0805103d` and then `7711da48` passed the runtime/tool transcript work but failed both runtimes on mobile UI positioning; the harness context panel opened offscreen when the desktop chip/panel remained mounted at a 390px viewport. The local fix now clamps the non-compact context popover to the viewport with fixed positioning derived from the trigger button, avoiding reliance on arbitrary/responsive Tailwind variants. Local UI typecheck, header/render architecture tests (`26 passed`), scoped diff check, and a Tailwind base-utility generation check passed. Deployed focused writes/default-approval (`4 passed`), context/native-compact (`2 passed`), and plan/heartbeat/automation (`6 passed`) passed while awaiting UI redeploy.
