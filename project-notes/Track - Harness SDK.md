...
status: active
created: 2026-04-26
updated: 2026-04-30 (Codex execution-surface hardening)
---
# Track - Harness SDK
...
- 2026-04-30 Codex native-coding priority hardening: screenshot review showed a severe Codex failure mode where a normal repo task did not orient from cwd/`AGENTS.md`/repo docs and instead drifted into Spindrel bridge-tool discovery. Every harness turn now gets an instruction-priority `native_workspace` hint ahead of the channel prompt, telling Claude/Codex to treat the resolved cwd as the native coding surface, read repo instruction files first, and use Spindrel host tools only for explicit host-state needs. Codex dynamic tools are now exported with `deferLoading: true`, and Codex bridge guidance is appended after the user's request instead of leading the prompt; it marks Spindrel tools as supplemental, exhaustive, and not the primary coding surface. Native Codex `commandExecution` results that look like sandbox/namespace infrastructure failures still mark the turn as an execution-surface error while ordinary command failures remain model-visible. Shared tool-not-found fallbacks discourage alias probing and tell agents to use exact listed tools or report the missing capability. Focused regressions cover native-workspace hint ordering, channel prompt preservation, Codex prompt ordering, deferred dynamic tools, namespace-failure abort classification, ordinary command errors, tool-error fallbacks, and the oversized Codex app-server JSON-RPC line reader.
