...
status: active
updated: 2026-04-29 (dashboard pin config NUX)
---
...
- **Widget agency receipts shipped** (2026-04-29) — bot-applied channel-dashboard changes now persist as `widget_agency_receipts` with action, reason, affected pins, compact before/after snapshots, bot/session/correlation/task metadata, and a channel API. Dashboard mutation tools accept an optional `reason` and record receipts only for channel dashboards. The usefulness UI deliberately calls read-only findings **widget proposals** and separately shows recent **bot widget changes**, avoiding confusion with global proposal/review surfaces.
