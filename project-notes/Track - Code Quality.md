...
status: active
created: 2026-04-09
updated: 2026-04-29 (task run host seam shipped)
---
...
Fresh scan used the `improve-codebase-architecture` deep Module lens: Interface, Implementation, Depth, Seam, Adapter, Leverage, and Locality. Architecture/drift slice passed before implementation (`45 passed`); focused harness/startup/slash slice passed after implementation (`37 passed, 21 skipped`). Platform slash cleanup follow-up passed focused slash client/drift tests (`10 passed`), broader slash integration slice (`12 passed, 26 skipped`), and redirected bytecode compile for Slack/Discord slash files. Task-run host follow-up passed the focused task worker unit group (`56 passed, 43 skipped`), task host architecture guard (`4 passed`), task integration profile (`16 skipped`), redirected bytecode compile, and scoped diff check.
...
**Live queue after the task-run host pass:**
1. **No active task-run blocker.** General task execution is split behind `task_run_host`; `app.agent.tasks` still intentionally owns schedules, subscriptions, event triggers, specialized runner dispatch, stuck-task recovery, stalled-workflow recovery, and the polling worker.
2. **Next verify-first architecture target.** Re-scan before choosing; likely candidates are remaining `app.agent.tasks` schedule/subscription/recovery surfaces or another hot file that shows fresh Locality pressure, not a preselected stale cluster.
