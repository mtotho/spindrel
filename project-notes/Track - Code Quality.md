...
status: active
created: 2026-04-09
updated: 2026-04-29 (task worker host seam shipped)
---
...
Fresh scan used the `improve-codebase-architecture` deep Module lens: Interface, Implementation, Depth, Seam, Adapter, Leverage, and Locality. Architecture/drift slice passed before implementation (`45 passed`); focused harness/startup/slash slice passed after implementation (`37 passed, 21 skipped`). Platform slash cleanup follow-up passed focused slash client/drift tests (`10 passed`), broader slash integration slice (`12 passed, 26 skipped`), and redirected bytecode compile for Slack/Discord slash files. Task-run host follow-up passed the focused task worker unit group (`56 passed, 43 skipped`), task host architecture guard (`4 passed`), task integration profile (`16 skipped`), redirected bytecode compile, and scoped diff check. Task-trigger host follow-up passed architecture guard (`4 passed`), non-cron trigger/task unit slice (`59 passed, 18 skipped`), cron subscription slice under `.venv` (`8 passed`), recovery safety slice (`66 passed, 25 skipped`), integration task profile (`16 skipped`), redirected bytecode compile, and scoped diff check. Task-worker host follow-up passed architecture guard (`4 passed`), recovery safety slice (`73 passed, 43 skipped`), task wrapper/host slice (`28 passed`), task integration profile (`16 skipped`), redirected bytecode compile, and scoped diff check.
...
**Live queue after the task-trigger host pass:**
1. **No active task worker blocker.** Task run, trigger, worker, and recovery policy are split behind host Modules; `app.agent.tasks` still intentionally owns specialized runner dispatch, exec tasks, heartbeat helper callbacks, and compatibility wrappers.
2. **Next verify-first architecture target.** Re-scan before choosing; likely candidates are `run_exec_task` as the last large task-side execution island, or a different hot file such as context assembly if fresh Locality pressure is higher.
