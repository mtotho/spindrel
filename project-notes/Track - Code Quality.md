...
status: active
created: 2026-04-09
updated: 2026-04-29 (task trigger host seam shipped)
---
...
Fresh scan used the `improve-codebase-architecture` deep Module lens: Interface, Implementation, Depth, Seam, Adapter, Leverage, and Locality. Architecture/drift slice passed before implementation (`45 passed`); focused harness/startup/slash slice passed after implementation (`37 passed, 21 skipped`). Platform slash cleanup follow-up passed focused slash client/drift tests (`10 passed`), broader slash integration slice (`12 passed, 26 skipped`), and redirected bytecode compile for Slack/Discord slash files. Task-run host follow-up passed the focused task worker unit group (`56 passed, 43 skipped`), task host architecture guard (`4 passed`), task integration profile (`16 skipped`), redirected bytecode compile, and scoped diff check. Task-trigger host follow-up passed architecture guard (`4 passed`), non-cron trigger/task unit slice (`59 passed, 18 skipped`), cron subscription slice under `.venv` (`8 passed`), recovery safety slice (`66 passed, 25 skipped`), integration task profile (`16 skipped`), redirected bytecode compile, and scoped diff check.
...
1. **No active task trigger blocker.** Schedules, channel subscriptions, and event triggers are split behind `task_trigger_host`; `app.agent.tasks` still intentionally owns specialized runner dispatch, exec tasks, stuck-task recovery, stalled-workflow recovery, and the polling worker.
2. **Next verify-first architecture target.** Re-scan before choosing; likely candidates are `app.agent.tasks` recovery/worker-loop surfaces, `run_exec_task`, or a different hot file such as context assembly if fresh Locality pressure is higher.
