...
- [x] **Phase 4F - Guided coding-run launch surface** — Added the Project coding-run API/service around the `project_coding_run` preset. Launch now validates Project channel membership, generates a branch/base/repo/runtime handoff, stores it in task `execution_config.project_coding_run`, creates a normal pending task with fresh-instance/new-session policy, and exposes `/coding-runs` as the Runs tab review cockpit with task, branch, activity, receipt, and handoff links. Screenshot staging now starts or shims a coding run before receipt capture; the 18000 e2e backend still returned 404 for the new endpoint during local-source visual verification, so the capture used the spec fallback for that list while the receipt API stayed real.
...
- Agent coding runs: deploy the guided `/coding-runs` endpoint to the 18000 e2e server, then recapture without the endpoint fallback and add automatic branch/MR helpers plus richer run progress before relying on this for unattended feature work.
