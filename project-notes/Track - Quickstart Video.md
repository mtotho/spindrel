...
status: active
updated: 2026-04-24 (A3-core-2a: kb-detail shipped; chat-content captures still queued)
---
...
| A3-core-2b | A | Core-feature heroes — chat-content sub-pass (4 captures) | **next** — `chat-delegation` (DelegationCard via `delegate_to_agent`), `chat-command-execution` (`exec_command` stdout/stderr card), `chat-plan-card` (`publish_plan` artifact), `chat-subagents` (`spawn_subagents` parallel results). Each needs distinct `message.metadata` shape synthesis (`assistant_turn_body` items + `tool_calls[]` ids + `tool_results[]` envelopes all consistent), best done one capture at a time with visual verification — modern OrderedTranscript renderer is fragile when shapes are subtly off. Two paths considered: (1) live agent loop with `/chat` + task polling + tool-fired assertion (real but ~$0.10 in LLM cost per run, flaky if the model picks a different tool); (2) synthetic injection via per-capture server helper (deterministic, no cost, but requires getting renderer shapes right). Recommend (2). All four chat captures double as B-roll for the quickstart video. |
