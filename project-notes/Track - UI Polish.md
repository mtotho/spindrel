...
- [x] Removed channel-settings autosave from long-lived config drafts: channel settings now keep local changes until explicit Save/Revert, and Heartbeat has its own Save/Revert controls so prompt text is not mutated by background PATCH/refetch cycles while typing.
