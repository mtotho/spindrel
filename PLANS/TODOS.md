...
7. Review direct DB access pattern in agent tools (todos.py, search_history.py, plans.py) — decide if a service layer is warranted vs current async_session() direct usage.
8. Sketch out Home Assistant + Frigate integrations — HAOS MCP tools already exist, Frigate sends events via MQTT. Brainstorm what useful agent integrations could look like.
9. Revisit GitHub Slack app thread replies — bot currently can't see them.
10. Resume GitHub webhook integration (`integrations/github/`) when server has a public URL.
