...
# session_id (str) → {carapace_id: approval_timestamp}
_approved: dict[str, dict[str, float]] = {}
...
def approve(session_id: str, carapace_id: str) -> None:
    """Record that user approved this capability for this session."""
    if session_id not in _approved:
        _approved[session_id] = {}
    _approved[session_id][carapace_id] = time.monotonic()
    logger.debug(
        "Capability approved: %s for session %s",
        carapace_id, session_id[:8],
    )


def is_approved(session_id: str | None, carapace_id: str) -> bool:
    """Check if user already approved this capability in this session."""
    if not session_id:
        return False
    return carapace_id in _approved.get(session_id, {})


def clear_session(session_id: str) -> int:
    """Remove all activations and approvals for a session. Returns count removed."""
...
def cleanup_stale() -> int:
    ...
    for sid in stale_sessions:
        del _sessions[sid]
        _approved.pop(sid, None)
    # Also clean approval-only entries (session cleared but approval lingers)
    stale_approvals = [
        sid for sid, caps in _approved.items()
        if sid not in _sessions and all(ts < cutoff for ts in caps.values())
    ]
    for sid in stale_approvals:
        del _approved[sid]
    return len(stale_sessions) + len(stale_approvals)
