...
def render_base_prompt(bot) -> str | None:
    ...
    variables = defaultdict(str, {
        ...
        "skills_section": "\n- **Skills**: Skill documents contain detailed procedures and reference material. Call `get_skill(skill_id)` to load a skill before responding to questions it covers. If a skill is already marked loaded in your skill index, do not fetch it again unless you intentionally need `refresh=true`. Skills marked as relevant in your skill index should be loaded before you respond. Use `get_skill_list()` to see all available skills — check it when you're unsure how to proceed." if has_skills else "",
