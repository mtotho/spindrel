...
class ProviderModel(Base):
    ...
    cached_input_cost_per_1m: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class UsageSpikeConfig(Base):
    ...
    target_ids: Mapped[list] = mapped_column(JSONB, server_default=text("'[]'::jsonb"))
...
class NotificationTarget(Base):
    """Admin-managed destination for human-facing notifications."""
    ...
    config: Mapped[dict] = mapped_column(JSONB, server_default=text("'{}'::jsonb"))
    enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text("true"))
    allowed_bot_ids: Mapped[list] = mapped_column(JSONB, server_default=text("'[]'::jsonb"))
    created_by: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(TIMESTAMP(timezone=True), server_default=text("now()"))
    updated_at: Mapped[datetime] = mapped_column(TIMESTAMP(timezone=True), server_default=text("now()"))

    __table_args__ = (
        Index("ix_notification_targets_kind", "kind"),
        Index("ix_notification_targets_enabled", "enabled"),
    )


class NotificationDelivery(Base):
    """Audit log for notification sends and per-destination delivery results."""
    __tablename__ = "notification_deliveries"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, server_default=text("gen_random_uuid()"))
    target_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("notification_targets.id", ondelete="SET NULL"), nullable=True,
    )
    root_target_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("notification_targets.id", ondelete="SET NULL"), nullable=True,
    )
    sender_type: Mapped[str] = mapped_column(Text, nullable=False)
    sender_id: Mapped[str | None] = mapped_column(Text, nullable=True)
    title: Mapped[str] = mapped_column(Text, nullable=False)
    body_preview: Mapped[str] = mapped_column(Text, nullable=False, server_default=text("''"))
    url: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class SystemHealthSummary(Base):
    """Deterministic daily rollup of server-side error evidence.

    Generated by ``app/services/system_health_summary.py`` once per day from
    the durable JSONL log + ``docker logs`` of allowlisted sibling
    containers + structured error sources (``trace_events``,
    ``tool_calls``). One row per run; ``attention_item_id`` points at the
    single rollup ``WorkspaceAttentionItem`` upserted alongside the row.
    """

    __tablename__ = "system_health_summaries"
    ...
    findings: Mapped[list] = mapped_column(JSONB, nullable=False, server_default=text("'[]'::jsonb"), default=list)
    source_counts: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default=text("'{}'::jsonb"), default=dict)
    attention_item_refs: Mapped[list] = mapped_column(JSONB, nullable=False, server_default=text("'[]'::jsonb"), default=list)
    trace_event_count: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("0"))
    tool_error_count: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("0"))
    attention_item_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)

    __table_args__ = (
        Index("ix_system_health_summaries_generated_at", text("generated_at DESC")),
    )
