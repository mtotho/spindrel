...
from app.db.models import (
    ...
    ConversationSection, Message, Session, Skill, Task, ToolApproval, ToolCall, TraceEvent,
    User,
)
...
from app.tools.local.search_history import _build_query, _build_session_query, _serialize_messages
...
@router.get("/{channel_id}/sessions", response_model=ChannelSessionListOut)
async def list_channel_sessions(
    ...
    """List channel sessions and the caller's scratch sessions for the picker."""
    channel = await db.get(Channel, channel_id)
    if not channel:
        raise HTTPException(status_code=404, detail="Channel not found")

    rows = await _build_session_search_rows(db, channel, auth, limit=limit)
    return ChannelSessionListOut(sessions=rows)


@router.get("/{channel_id}/sessions/search", response_model=ChannelSessionSearchOut)
async def search_channel_sessions(
    channel_id: uuid.UUID,
    ...
    """Search live messages and archived sections, grouped by session."""
    channel = await db.get(Channel, channel_id)
    if not channel:
        raise HTTPException(status_code=404, detail="Channel not found")
    ...
    if not row_by_id:
        return ChannelSessionSearchOut(query=q, sessions=[])

    matches_by_session: dict[uuid.UUID, list[ChannelSessionMatchOut]] = {}

    message_stmt = _build_session_query(row_by_id.keys(), query=q, role="all", limit=limit * 4)
    ...
    for session_id in row_by_id:
        try:
            section_matches = await search_sections(session_id, q)
        except Exception:
    ...
    return ChannelSessionSearchOut(query=q, sessions=ranked[:limit])
...
async def _build_session_search_rows(
    db: AsyncSession,
    channel: Channel,
    auth,
    *,
