...
from app.services.channel_sessions import (
    ChannelSessionListOut,
    ChannelSessionSearchOut,
...
async def search_channel_sessions(
    ...
    rows = await search_channel_session_rows(db, channel, auth, query=q, limit=limit)
    return ChannelSessionSearchOut(query=q, sessions=rows)
...
class SpatialBotPolicyRequest(BaseModel):
    ...
    allow_attention_beacons: Optional[bool] = None
    allow_map_view: Optional[bool] = None
