...
SETTINGS_SCHEMA: dict[str, dict[str, Any]] = {
    ...
    "MEMORY_FLUSH_ENABLED": {"group": "Memory & Learning", "label": "Memory Flush Before Compaction", "description": "Run a dedicated memory flush before compaction — bot saves memories/knowledge/persona while it still sees full context", "type": "bool"},
    "MEMORY_FLUSH_MODEL": {"group": "Memory & Learning", "label": "Memory Flush Model", "description": "Model for memory flush (empty = use bot's model)", "type": "string", "widget": "model"},
    "MEMORY_FLUSH_MODEL_PROVIDER_ID": {"group": "Memory & Learning", "label": "Memory Flush Model Provider", "type": "string", "description": "Provider for memory flush model", "ui_hidden": True},
    "MEMORY_FLUSH_DEFAULT_PROMPT": {"group": "Memory & Learning", "label": "Memory Flush Default Prompt", "description": "Default prompt for the memory flush pass. Tells the bot what to save before context is archived.", "type": "string", "widget": "textarea"},
    "MEMORY_SCHEME_PROMPT": {"group": "Memory & Learning", "label": "Memory Scheme System Prompt (Override)", "description": "Custom override for the workspace-files memory prompt. Leave empty to use the built-in default.", "type": "string", "widget": "textarea", "nullable": True, "ui_hidden": True},
    "MEMORY_SCHEME_FLUSH_PROMPT": {"group": "Memory & Learning", "label": "Memory Scheme Flush Prompt (Override)", "description": "Custom override for the workspace-files flush prompt. Leave empty to use the built-in default.", "type": "string", "widget": "textarea", "nullable": True, "ui_hidden": True},
    "PREVIOUS_SUMMARY_INJECT_CHARS": {"group": "Memory & Learning", "label": "Previous Summary Max Chars", "description": "Max characters of existing summary injected into heartbeat/memory-flush context. Truncates at sentence boundary.", "type": "int", "min": 0, "max": 5000},
    ...
    # --- Memory & Learning: Dreaming ---
    "MEMORY_HYGIENE_ENABLED": {"group": "Memory & Learning", "label": "Memory Hygiene Enabled", "description": "Enable periodic memory hygiene jobs for workspace-files bots. Bots review cross-channel memory, promote stable facts, prune stale entries, and consolidate skills.", "type": "bool"},
    "MEMORY_HYGIENE_INTERVAL_HOURS": {"group": "Memory & Learning", "label": "Interval (hours)", "description": "Hours between hygiene runs (per-bot override available)", "type": "int", "min": 1, "max": 720},
    "MEMORY_HYGIENE_PROMPT": {"group": "Memory & Learning", "label": "Hygiene Prompt", "description": "Custom prompt for hygiene runs. Leave empty to use the built-in default shown below.", "type": "string", "widget": "textarea", "nullable": True, "builtin_default_key": "DEFAULT_MEMORY_HYGIENE_PROMPT"},
    "MEMORY_HYGIENE_ONLY_IF_ACTIVE": {"group": "Memory & Learning", "label": "Only If Active", "description": "Skip hygiene when no user messages have landed in any of a bot's channels (primary or member) since the last run. Bot-to-bot delegation, heartbeats, and assistant replies do NOT count — a bot whose channels only see bot traffic will never dream unless this is off.", "type": "bool"},
    "MEMORY_HYGIENE_MODEL": {"group": "Memory & Learning", "label": "Model", "description": "Default model for hygiene runs (empty = use each bot's default model). Per-bot override available.", "type": "string", "widget": "model", "nullable": True},
    "MEMORY_HYGIENE_MODEL_PROVIDER_ID": {"group": "Memory & Learning", "label": "Hygiene Model Provider", "type": "string", "description": "Provider for memory hygiene model", "ui_hidden": True},
    "MEMORY_HYGIENE_TARGET_HOUR": {"group": "Memory & Learning", "label": "Target Start Hour", "description": "Hour of day (0-23, local time) when hygiene runs should cluster. Bots stagger within ~60 min of this hour. Set -1 to disable (runs spread across the full interval).", "type": "int", "min": -1, "max": 23},
    "MEMORY_MD_NUDGE_THRESHOLD": {"group": "Memory & Learning", "label": "Memory Size Nudge (lines)", "description": "When MEMORY.md exceeds this many lines, the bot gets a system message each turn reminding it to prune and consolidate. Set to 0 to disable.", "type": "int", "min": 0, "max": 500},
    # --- Skill Review (separate dreaming job for skill curation) ---
    "SKILL_REVIEW_ENABLED": {"group": "Memory & Learning", "label": "Skill Review Enabled", "description": "Enable periodic skill review jobs. Cross-channel reflection, skill pruning, auto-inject audit. Runs separately from memory maintenance.", "type": "bool"},
    "SKILL_REVIEW_INTERVAL_HOURS": {"group": "Memory & Learning", "label": "Skill Review Interval (hours)", "description": "Hours between skill review runs (default: 72). Skill rot is slower than memory drift.", "type": "int", "min": 1, "max": 720},
    "SKILL_REVIEW_PROMPT": {"group": "Memory & Learning", "label": "Skill Review Prompt", "description": "Custom prompt for skill review runs. Leave empty to use the built-in default.", "type": "string", "widget": "textarea", "nullable": True, "builtin_default_key": "DEFAULT_SKILL_REVIEW_PROMPT"},
    "SKILL_REVIEW_ONLY_IF_ACTIVE": {"group": "Memory & Learning", "label": "Skill Review Only If Active", "description": "Skip skill review when no user messages since last run. Recommended: off (skill rot happens regardless of activity).", "type": "bool"},
    "SKILL_REVIEW_MODEL": {"group": "Memory & Learning", "label": "Skill Review Model", "description": "Default model for skill review runs (empty = use bot's model). Should be a strong reasoning model.", "type": "string", "widget": "model", "nullable": True},
    "SKILL_REVIEW_MODEL_PROVIDER_ID": {"group": "Memory & Learning", "label": "Skill Review Model Provider", "type": "string", "description": "Provider for skill review model", "ui_hidden": True},
    "SKILL_REVIEW_TARGET_HOUR": {"group": "Memory & Learning", "label": "Skill Review Target Hour", "description": "Hour of day (0-23, local time) when skill review runs should cluster. Set -1 to disable.", "type": "int", "min": -1, "max": 23},
    # --- Heartbeat ---
