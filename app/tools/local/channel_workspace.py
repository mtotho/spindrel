...
def _get_bot_and_roots(channel_id: str | None = None) -> tuple:
    ...
    ws_root = str(Path(_get_ws_root(bot)).resolve())
...
async def search_channel_archive(query: str) -> str:
    ...
    bot, ch_id, ws_root, embedding_model = _get_bot_and_roots()
...
async def search_channel_workspace(query: str, channel_id: str | None = None) -> str:
    ...
    bot, ch_id, ws_root, embedding_model = _get_bot_and_roots(channel_id)
    if not bot or not ch_id:
...
async def list_workspace_channels() -> str:
    ...
    if not rows:
        return "No other channels with workspace enabled."

    lines = []
    for ch_id, name, client_id in rows:
        ch_str = str(ch_id)
