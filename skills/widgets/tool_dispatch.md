...
description: How widgets trigger backend work — `window.spindrel.callTool` and the underlying `POST /api/v1/widget-actions` envelope. Three dispatch types (tool/api/widget_config), envelope shape, truncation rules, and dispatch vs `spindrel.api`.
