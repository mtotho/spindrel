...
| Saving a YAML tool-widget after only reading the template | `check_widget_authoring(..., include_runtime=true)` with a representative `sample_payload` | The shared authoring check validates YAML/Python/schema, renders the preview envelope, runs static health, and opens the draft in the real widget host when browser runtime is configured. |
