...
function MemorySection({ draft, update, botId }: {
  draft: BotConfig;
  ...
  const selectMode = (mode: string | null) => {
    if (mode === "workspace-files") {
      update({
        ...
        // Disable DB memory/knowledge — workspace files replaces both
...
└── reference/
    └── *.md               # Longer docs, searchable + readable via tools`}</pre>
          </div>
...
- get_memory_file(name) — read a specific memory file by name
...
- Write important learnings immediately, don't wait for session end`}</pre>
              </div>
            )}
          </div>
        </>
...
// ---------------------------------------------------------------------------
function KnowledgeSection({ draft, update }: {
  draft: BotConfig;
  update: (p: Partial<BotConfig>) => void;
}) {
  const isWorkspaceFiles = draft.memory_scheme === "workspace-files";
