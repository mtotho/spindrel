...
import {
  canonicalizePaletteHref,
  normalizePalettePathInput,
  resolvePaletteRoute,
} from "./paletteRoutes.js";
...
test("canonicalizePaletteHref normalizes route aliases to their durable targets", () => {
  ...
  assert.equal(canonicalizePaletteHref("/canvas?node=node-1"), "/spatial?node=node-1");
  assert.equal(canonicalizePaletteHref("/?channel=channel-1"), "/spatial?channel=channel-1");
  assert.equal(
    canonicalizePaletteHref("/admin/widget-packages/pkg-1"),
    "/widgets/dev?id=pkg-1#templates",
  );
  assert.equal(
...
test("normalizePalettePathInput accepts both raw app paths and copied app urls", () => {
  assert.equal(
    normalizePalettePathInput("/widgets/dev?id=widget-1#templates"),
    "/widgets/dev?id=widget-1#templates",
  );
  assert.equal(
    normalizePalettePathInput("https://app.example.test/admin/logs/trace-1"),
    "/admin/logs/trace-1",
  );
});
