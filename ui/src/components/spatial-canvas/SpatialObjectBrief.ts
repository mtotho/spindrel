import type { WorkspaceMapObjectState, WorkspaceMapSignal } from "../../api/types/workspaceMapState";

export type ObjectBriefTone = "danger" | "warning" | "active" | "muted";

export interface SpatialObjectBrief {
  headline: string;
  summary: string;
  tone: ObjectBriefTone;
  sourceLines: string[];
...
function titleForSignal(signal?: WorkspaceMapSignal | null): string | null {
...
function sourceLinesFor(state: WorkspaceMapObjectState): string[] {
  const source = state.source || {};
  const attached = state.attached || {};
  const lines: string[] = [];
  if (state.kind === "channel") {
    const bot = asString(source.primary_bot_name) || asString(source.primary_bot_id);
    if (bot) lines.push(`Primary bot: ${bot}`);
    if (state.counts.bots > 1) lines.push(`${state.counts.bots} bots attached`);
    if (state.counts.integrations > 0) lines.push(`${state.counts.integrations} integrations`);
    const heartbeat = attached.heartbeat as Record<string, unknown> | undefined;
    if (heartbeat) {
    ...
    if (channelIds) lines.push(`${channelIds} rooms`);
    ...
    if (sourceChannel) lines.push(`Source: #${sourceChannel}`);
    const cronCount = asNumber(attached.cron_count) ?? 0;
    const eventCount = asNumber(attached.event_count) ?? 0;
    if (cronCount || eventCount) lines.push(`${cronCount} cron · ${eventCount} event`);
