...
import {
  buildRecentHref,
  migrateRecentPage,
  shouldSkipRecentPage,
} from "../../lib/recentPages.js";
import { resolveRecentPaletteItem } from "./recent.js";

function makeRecent(href: string, label?: string): RecentPage {
  return { href, label };
}
...
test("buildRecentHref preserves search and hash", () => {
  assert.equal(
    buildRecentHref("/channels/channel-1/session/session-1", "?scratch=true", "#notes"),
    "/channels/channel-1/session/session-1?scratch=true#notes",
  );
});
...
test("resolveRecentPaletteItem formats titled session recents without adding a guid prefix", () => {
  const resolved = resolveRecentPaletteItem(
    makeRecent(
      ...
      "Inbox cleanup · #quality-assurance",
    ),
    [],
    { channelNameById: new Map([["channel-1", "quality-assurance"]]) },
  );
...
test("shouldSkipRecentPage treats a full href with search as the current page", () => {
  const currentHref = buildRecentHref(
    "/channels/channel-1/session/session-1",
    "?scratch=true",
  );
  assert.equal(
    shouldSkipRecentPage(
...
test("resolveRecentPaletteItem still prefers exact palette items when present", () => {
  ...
  const resolved = resolveRecentPaletteItem(
    makeRecent("/channels/channel-1", "stale label"),
    [exactItem],
    { channelNameById: new Map([["channel-1", "quality-assurance"]]) },
  );
