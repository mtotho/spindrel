...
import type { OrderedTurnBodyItem, SharedToolTranscriptEntry } from "./toolTranscriptModel.js";

function entry(id: string): SharedToolTranscriptEntry {
  return {
    id,
    kind: "activity",
    label: id,
...
test("groups adjacent transcript items into one row run", () => {
  ...
  assert.deepEqual(
    grouped[0].kind === "transcript" ? grouped[0].entries.map((x) => x.id) : [],
    ["a1", "b1", "b2"],
  );
});
