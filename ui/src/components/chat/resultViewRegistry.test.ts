...
import {
  ...
  contentTypeToViewKey,
  envelopeViewKey,
  type ResultViewRendererProps,
} from "./resultViewRegistry.js";
import { getChatModeConfig, isTranscriptFlowComposer } from "./chatModes.js";
import type { ToolResultEnvelope } from "../../types/api";

interface TestProps extends ResultViewRendererProps {
  label: string;
}

function envelope(fields: Partial<ToolResultEnvelope>): ToolResultEnvelope {
  return {
...
test("result view registry resolves exact mode and never falls back to default for another mode", () => {
  const registry = createResultViewRegistry<TestProps>();
