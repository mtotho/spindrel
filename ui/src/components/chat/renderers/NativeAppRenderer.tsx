...
function NativePlanQuestionsWidget({
  envelope,
  t,
}: {
  envelope: ToolResultEnvelope;
  ...
  const pushToComposer = async () => {
    const text = buildAnswerText();
    if (!text) return;
    window.dispatchEvent(new CustomEvent("spindrel:plan-question-fill", { detail: { text } }));
    try {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <div style={{ fontSize: 12, color: error ? t.danger : t.textDim }}>
          {error ?? (copied ? "Added to composer and copied to clipboard." : "This fills the composer so you can review and send the answers normally.")}
        </div>
        <button
          type="button"
          onClick={pushToComposer}
          style={{
            borderRadius: 999,
...
export function NativeAppRenderer({
  ...
  switch (payload.widget_ref) {
    case "core/plan_questions":
      return (
        <NativePlanQuestionsWidget
          envelope={envelope}
          t={t}
        />
      );
