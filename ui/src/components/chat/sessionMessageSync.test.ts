...
import {
  mergePersistedAndSyntheticMessages,
  shouldKeepSyntheticAssistantMessage,
} from "./sessionMessageSync.js";

function makeSyntheticWidgetMessage(): Message {
  return {
    id: "turn-1",
...
test("synthetic widget rows survive when the fetched DB row is structurally weaker", () => {
  ...
  assert.deepEqual(
    mergePersistedAndSyntheticMessages([dbRow], [synthetic]).map((message) => message.id),
    ["db-1", "turn-1"],
  );
});
