...
export function shouldGroup(current, prev) {
    if (!prev)
        return false;
    if (current.role !== prev.role)
        return false;
    // Don't group bot response with preceding trigger card
    if (prev.role === "user" && prev.metadata?.trigger)
        return false;
 ...
 */
export function getTurnText(invertedData, index) {
...
export function getTurnMessages(invertedData, index) {
    ...
    for (let i = index - 1; i >= 0; i--) {
        ...
        if (!shouldGroup(msg, invertedData[i + 1]))
            break;
        messages.push(msg);
    }
    return messages;
}
export function stringifyTurnMessages(messages) {
    if (!messages?.length)
        return undefined;
