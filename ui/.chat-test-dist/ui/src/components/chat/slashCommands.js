...
export function buildCompletedSlashCommandText(commandId, arg) {
...
export function resolveSlashCommand(raw, surface, catalog, availableIds) {
...
export function detectMissingSlashArgs(raw, surface, catalog, availableIds) {
    const trimmed = raw.trim();
    if (!trimmed.startsWith("/") || trimmed.includes("\n"))
        return null;
    const tokens = trimmed.slice(1).split(/\s+/).filter((t) => t.length > 0);
...
export function findSpec(catalog, id) {
    return catalog.find((c) => c.id === id) ?? null;
}
export function buildSlashCommandResultMessage(result, sessionId, channelId) {
    const now = new Date().toISOString();
    const content = result.result_type === "harness_native_compaction"
...
export function buildSlashCommandPendingMessage(commandId, sessionId, channelId) {
    const now = new Date().toISOString();
    return {
