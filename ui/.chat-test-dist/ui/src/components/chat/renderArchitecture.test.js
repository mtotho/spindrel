...
function readChatSessionSourceModes() {
...
test("channel route renders desktop session tabs through dedicated components and pure model", () => {
    const channelRoute = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/index.tsx"), "utf8");
    const sessionTabs = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/ChannelSessionTabs.tsx"), "utf8");
    const sessionSurfaces = readFileSync(resolve(process.cwd(), "src/lib/channelSessionSurfaces.ts"), "utf8");
