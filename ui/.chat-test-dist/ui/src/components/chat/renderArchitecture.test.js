...
test("native plan replay hydrates out-of-line envelopes before rendering plan cards", () => {
    ...
    const sessionPlanCard = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/SessionPlanCard.tsx"), "utf8");
...
test("harness channel settings keep the channel prompt editor visible", () => {
    const settingsSections = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/ChannelSettingsSections.tsx"), "utf8");
    ...
    assert.match(settingsSections.slice(harnessBranchStart, harnessRoutingSection), /<ChannelPromptSection\s+form=\{form\}/);
});
...
test("session resume cards do not show normal bot model labels for harness sessions", () => {
    ...
    const approvalsHook = readFileSync(resolve(process.cwd(), "src/api/hooks/useApprovals.ts"), "utf8");
    const channelPage = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/index.tsx"), "utf8");
    const chatSessionSources = readChatSessionSourceModes();
...
test("channel route delegates workbench chrome orchestration to its local controller", () => {
    const channelRoute = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/index.tsx"), "utf8");
    const workbenchController = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/useChannelWorkbenchController.tsx"), "utf8");
...
test("channel route renders desktop session tabs through dedicated components and pure model", () => {
    const channelRoute = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/index.tsx"), "utf8");
    const topTabsController = readFileSync(resolve(process.cwd(), "app/(app)/channels/[channelId]/useChannelTopTabsController.ts"), "utf8");
