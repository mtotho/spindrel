 ...
 */
export function stripLegacyIngestPrefix(content, source) {
    if (!content)
        return content;
    if (source === "slack")
        return content.replace(LEGACY_SLACK_PREFIX_RE, "");
    if (source === "discord")
        return content.replace(LEGACY_DISCORD_PREFIX_RE, "");
    if (source === "bluebubbles")
        return content.replace(LEGACY_NAME_PREFIX_RE, "");
    return content;
}
...
export function resolveDisplay(message, botName) {
...
export function terminalTranscriptRole(message) {
    if (message.role === "assistant")
        return "assistant";
