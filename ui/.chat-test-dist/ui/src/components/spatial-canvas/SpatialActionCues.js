...
export function SpatialActionCueLayer({ objects, selectedObjectId, highlightedObjectId, scale, suppressedObjectIds, }) {
    ...
    return (_jsx(_Fragment, { children: visible.map((item) => {
            ...
            return (_jsx("div", { "data-testid": "spatial-action-cue-marker", "data-spatial-action-cue-id": item.id, "data-spatial-action-cue-intent": intent, title: cueTitle(item), className: "pointer-events-none absolute z-[4996]", style: {
                ...
                }, children: _jsx("div", { className: `relative transition-transform duration-150 ${highlighted ? "scale-110" : "scale-100"}`, style: { width: 0, height: 0 }, children: _jsxs("div", { "aria-hidden": "true", className: `absolute flex h-5 w-5 items-center justify-center rounded-full ring-1 backdrop-blur-sm ${toneClass} ${highlighted ? "ring-2 ring-accent/35" : ""}`, style: { transform: `translate(${badgeX}px, ${badgeY}px)` }, children: [_jsx(Icon, { size: 13 }), _jsx("span", { className: "sr-only", children: cueLabel(item) })] }) }) }, `action-cue-${item.id}`));
        }) }));
}
