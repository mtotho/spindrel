...
export function scorePaletteSearchItems(items, query, recencyBonus = new Map(), searchLimit = 30) {
    return items
        ...
        const [recentSessionChannelScore] = item.category === "Recent"
 ...
 */
export function usePaletteSearch(allItems, query, { currentHref, recentLimit = 20, searchLimit = 30, isAdmin = true, surface = null, onMapChannelIds, }) {
    ...
    const scored = useMemo(() => {
        ...
        const allItemsByHref = new Set(sourceItems.map((it) => it.href).filter((href) => typeof href === "string"));
