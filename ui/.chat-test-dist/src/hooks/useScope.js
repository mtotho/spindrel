...
export function hasScope(keyScopes, required) {
    ...
    for (const s of keyScopes) {
        ...
        if (reqResource.startsWith(sResource + ".")) {
            if (sAction === reqAction)
                return true;
            if (sAction === "write" && reqAction === "read")
                return true;
        }
...
export function useScopes(...scopes) {
    ...
    return scopes.every((s) => hasScope(userScopes, s));
}
/** Any-of: true iff AT LEAST ONE of the given scopes is granted. */
export function useAnyScope(...scopes) {
    const user = useAuthStore((s) => s.user);
    if (!user)
        return false;
    const userScopes = user.scopes ?? [];
    return scopes.some((s) => hasScope(userScopes, s));
}
