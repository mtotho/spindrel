...
export function loadDensityIntensity(storage = localStorage) {
    try {
        const v = storage.getItem(DENSITY_INTENSITY_KEY);
        if (v === "off" || v === "subtle" || v === "bold")
            return v;
    }
    catch {
        /* storage disabled */
...
export function loadDensityWindow(storage = localStorage) {
    try {
...
export function loadDensityCompare(storage = localStorage) {
    try {
...
export function loadDensityAnimate(storage = localStorage) {
    try {
        if (storage.getItem(DENSITY_ANIMATE_KEY) === "0")
            return false;
    }
    catch {
        /* storage disabled */
...
export function loadConnectionsEnabled(storage = localStorage) {
    try {
...
export function loadBotsVisible(storage = localStorage) {
    try {
...
export function loadBotsReduced(storage = localStorage) {
    try {
...
export function loadTrailsMode(storage = localStorage) {
    try {
        const v = storage.getItem(TRAILS_MODE_KEY);
        if (v === "off" || v === "hover" || v === "all")
            return v;
    }
    catch {
        /* storage disabled */
...
export function loadMinimapVisible(storage = localStorage) {
    try {
...
export function loadLandmarkBeaconsVisible(storage = localStorage) {
    try {
...
export function loadAttentionSignalsVisible(storage = localStorage) {
    try {
 ...
 */
export function getViewportWorldBbox(camera, viewport, marginPx = 0) {
...
export function parseStoredCamera(raw) {
    if (!raw)
...
export function loadStoredCamera(storage = localStorage) {
    try {
...
export function projectFisheye(worldCx, worldCy, camera, focalScreen, lensRadius) {
    if (lensRadius <= 0)
        return { dxWorld: 0, dyWorld: 0, sizeFactor: 1 };
    ...
    const sizeFactor = Math.max(LENS_MIN_SCALE, Math.min(1, Math.pow(ratio, LENS_SIZE_EXP)));
    return {
