export const HISTORY_MODE_META = [
    {
        value: "file",
        label: "File",
        accentColor: "#d97706",
        summary: "Active default",
        detail: "Conversation is archived into titled, searchable sections stored in the database. " +
    ...
    },
    {
        ...
        detail: "Each compaction replaces the previous summary with a new one. " +
    ...
    },
    {
        ...
        detail: "Conversation is archived into titled sections with embeddings, and the system attempts " +
...
export function getHistoryModeMeta(mode) {
    return HISTORY_MODE_META.find((entry) => entry.value === mode) ?? HISTORY_MODE_META[0];
}
export function historyModeOptionLabel(mode) {
    if (mode.recommended)
        return `${mode.label} (active default)`;
    if (mode.legacy)
        return `${mode.label} (legacy)`;
    return mode.label;
}
