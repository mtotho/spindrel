...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --tier)
            TIER="${2:?--tier requires one of: core, questions, publish, approve, answers, progress, replan, guardrails, replay, behavior, quality, stress, adherence}"
            shift 2
            ;;
        --tier=*)
            TIER="${1#--tier=}"
...
export E2E_BOT_ID="${E2E_BOT_ID:-$SPINDREL_PLAN_BOT_ID}"
