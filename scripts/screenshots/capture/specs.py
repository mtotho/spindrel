...
_ASSERT_MAP_BRIEF_SELECTION_JS = (
    ...
    "if (!/Needs action/.test(compass.textContent || '')) throw new Error('action compass does not identify the active action queue');"
    "if (!compass.querySelector('[data-spatial-action-compass-selected=\"true\"]')) throw new Error('action compass does not agree with selected Map Brief object');"
